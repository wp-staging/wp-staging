...
class WpDefaultDirectories
{
    ...
    {
        $relPath = str_replace($this->strUtils->sanitizeDirectorySeparator(ABSPATH), null, $this->getUploadsPath());
    ...
    {
        $relPath = str_replace(ABSPATH, null, WP_CONTENT_DIR);
    ...
    {
        $relPath = str_replace(ABSPATH, null, WP_PLUGIN_DIR);
    ...
    public function getWpCoreDirectories()
    {
        ...
        foreach ($directories as $directory) {
            ...
            $shouldSkip = ($name !== 'wp-admin' &&
                ...
                $name !== 'wp-content' &&
                $name !== 'sites') &&
                strpos(strrev($path), strrev(ABSPATH . "wp-admin")) === false &&
                strpos(strrev($path), strrev(ABSPATH . "wp-includes")) === false &&
                strpos(strrev($path), strrev(ABSPATH . "wp-content")) === false;
        ...
        // Gather Plugins
        $pluginsDir = ABSPATH . $this->getRelativePluginPath();
        $this->handleSelfAndSubDirs($pluginsDir);

        // Gather Themes
        $themesDir = ABSPATH . $this->getRelativeThemePath();
