 ...
 *
 * Rendered server-side when the banner was previously dismissed. The same markup is built in
 * JavaScript by src/assets/js/src/modules/cli-dock-cta.js after the shrink-to-dock animation.
 *
 * Structure: Vertical container wraps button + sublabel so the sublabel
 * aligns under the button only (not centered under entire row).
 *
 * License gating: Pulling this site directly into a local Docker environment
 * requires a Developer or Agency license. Everyone else sees a "Developer"
 * badge and the button opens the upgrade modal instead of performing the action.
 */
...
<div class="wpstg-cli-dock-cta-wrapper !wpstg-flex wpstg-flex-col wpstg-items-center">
    <button type="button"
        ...
        aria-label="<?php esc_attr_e('Pull Site to Local', 'wp-staging'); ?>"
    >
        ...
        <span class="wpstg-badge wpstg-badge-pro"><?php esc_html_e('Developer', 'wp-staging'); ?></span>
        <?php endif; ?>
    </button>
    <span class="wpstg-text-xs wpstg-leading-[0.8] wpstg-text-gray-500 dark:wpstg-text-gray-400 wpstg-mt-1.5"><?php esc_html_e('Direct via CLI', 'wp-staging'); ?></span>
</div>
