...
class FnDispatcher
{
    ...
    private function fn_avg(array $args)
    {
        ...
        $sum = $this->reduce('avg:0', $args[0], ['number'], function ($a, $b) {
            return \WPStaging\Vendor\JmesPath\Utils::add($a, $b);
        });
    ...
    private function fn_sort(array $args)
    {
        ...
        $valid = ['string', 'number'];
        return \WPStaging\Vendor\JmesPath\Utils::stableSort($args[0], function ($a, $b) use($valid) {
    ...
    private function fn_sort_by(array $args)
    {
        ...
        $expr = $args[1];
        $valid = ['string', 'number'];
        return \WPStaging\Vendor\JmesPath\Utils::stableSort($args[0], function ($a, $b) use($expr, $valid) {
            $va = $expr($a);
            $vb = $expr($b);
    ...
    private function fn_type(array $args)
    {
        ...
        return \WPStaging\Vendor\JmesPath\Utils::type($args[0]);
    }
    ...
    private function fn_to_number(array $args)
    {
        ...
        $value = $args[0];
        $type = \WPStaging\Vendor\JmesPath\Utils::type($value);
        if ($type == 'number') {
            return $value;
    ...
    private function fn_to_array(array $args)
    {
        ...
        return \WPStaging\Vendor\JmesPath\Utils::isArray($args[0]) ? $args[0] : [$args[0]];
    }
    ...
    {
        if ($types[0] == 'any' || \in_array(\WPStaging\Vendor\JmesPath\Utils::type($value), $types) || $value === [] && \in_array('object', $types)) {
            return;
        }
        $msg = 'must be one of the following types: ' . \implode(', ', $types) . '. ' . \WPStaging\Vendor\JmesPath\Utils::type($value) . ' found';
    ...
    {
        $ta = \WPStaging\Vendor\JmesPath\Utils::type($a);
        $tb = \WPStaging\Vendor\JmesPath\Utils::type($b);
        if ($ta !== $tb) {
