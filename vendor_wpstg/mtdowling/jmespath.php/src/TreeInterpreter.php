...
class TreeInterpreter
{
    ...
    {
        $this->fnDispatcher = $fnDispatcher ?: \WPStaging\Vendor\JmesPath\FnDispatcher::getInstance();
    }
    /**
    ...
    private function dispatch(array $node, $value)
    {
        ...
        switch ($node['type']) {
            ...
            case 'index':
                if (!\WPStaging\Vendor\JmesPath\Utils::isArray($value)) {
                    return null;
                }
            ...
            case 'projection':
                ...
                switch ($node['from']) {
                    case 'object':
                        if (!\WPStaging\Vendor\JmesPath\Utils::isObject($left)) {
                            return null;
                        }
                        break;
                    case 'array':
                        if (!\WPStaging\Vendor\JmesPath\Utils::isArray($left)) {
                            return null;
                        }
                        break;
                    default:
            ...
            case 'or':
                ...
                return \WPStaging\Vendor\JmesPath\Utils::isTruthy($result) ? $result : $this->dispatch($node['children'][1], $value);
            case 'and':
                $result = $this->dispatch($node['children'][0], $value);
                return \WPStaging\Vendor\JmesPath\Utils::isTruthy($result) ? $this->dispatch($node['children'][1], $value) : $result;
            case 'not':
                return !\WPStaging\Vendor\JmesPath\Utils::isTruthy($this->dispatch($node['children'][0], $value));
            case 'pipe':
                return $this->dispatch($node['children'][1], $this->dispatch($node['children'][0], $value));
            ...
            case 'comparator':
                ...
                if ($node['value'] == '==') {
                    return \WPStaging\Vendor\JmesPath\Utils::isEqual($left, $right);
                } elseif ($node['value'] == '!=') {
                    return !\WPStaging\Vendor\JmesPath\Utils::isEqual($left, $right);
                } else {
            ...
            case 'condition':
                return \WPStaging\Vendor\JmesPath\Utils::isTruthy($this->dispatch($node['children'][0], $value)) ? $this->dispatch($node['children'][1], $value) : null;
            case 'function':
                $args = [];
            ...
            case 'slice':
                return \is_string($value) || \WPStaging\Vendor\JmesPath\Utils::isArray($value) ? \WPStaging\Vendor\JmesPath\Utils::slice($value, $node['value'][0], $node['value'][1], $node['value'][2]) : null;
            case 'expref':
