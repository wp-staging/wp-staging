...
use Exception;
use WPStaging\Vendor\Monolog\Formatter\LineFormatter;
use WPStaging\Vendor\Monolog\Logger;
use WPStaging\Vendor\Monolog\Utils;
use WPStaging\Vendor\PhpConsole\Connector;
use WPStaging\Vendor\PhpConsole\Handler;
use WPStaging\Vendor\PhpConsole\Helper;
/**
 ...
 */
class PHPConsoleHandler extends \WPStaging\Vendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     * @throws Exception
     */
    public function __construct(array $options = array(), \WPStaging\Vendor\PhpConsole\Connector $connector = null, $level = \WPStaging\Vendor\Monolog\Logger::DEBUG, $bubble = \true)
    {
        if (!\class_exists('WPStaging\\Vendor\\PhpConsole\\Connector')) {
    ...
    private function initConnector(\WPStaging\Vendor\PhpConsole\Connector $connector = null)
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \WPStaging\Vendor\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \WPStaging\Vendor\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\WPStaging\Vendor\PhpConsole\Helper::isRegistered()) {
            \WPStaging\Vendor\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \WPStaging\Vendor\PhpConsole\Handler::getInstance();
    ...
    {
        if ($record['level'] < \WPStaging\Vendor\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record)
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \WPStaging\Vendor\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(\array_filter($record['context'])), null, \true);
        }
    ...
    {
        return new \WPStaging\Vendor\Monolog\Formatter\LineFormatter('%message%');
    }
}
