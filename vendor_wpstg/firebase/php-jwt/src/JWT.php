...
namespace WPStaging\Vendor\Firebase\JWT;
...
class JWT
{
    ...
    public static function decode($jwt, $keyOrKeyArray, array $allowed_algs = array())
    {
        ...
        if (!static::verify("{$headb64}.{$bodyb64}", $sig, $keyMaterial, $header->alg)) {
            throw new \WPStaging\Vendor\Firebase\JWT\SignatureInvalidException('Signature verification failed');
        }
        // Check the nbf if it is defined. This is the time that the
        // token can actually be used. If it's not yet that time, abort.
        if (isset($payload->nbf) && $payload->nbf > $timestamp + static::$leeway) {
            throw new \WPStaging\Vendor\Firebase\JWT\BeforeValidException('Cannot handle token prior to ' . \date(\DateTime::ISO8601, $payload->nbf));
        }
        // Check that this token has been created before 'now'. This prevents
        // using tokens that have been created for later use (and haven't
        // correctly used the nbf claim).
        if (isset($payload->iat) && $payload->iat > $timestamp + static::$leeway) {
            throw new \WPStaging\Vendor\Firebase\JWT\BeforeValidException('Cannot handle token prior to ' . \date(\DateTime::ISO8601, $payload->iat));
        }
        // Check if this token has expired.
        if (isset($payload->exp) && $timestamp - static::$leeway >= $payload->exp) {
            throw new \WPStaging\Vendor\Firebase\JWT\ExpiredException('Expired token');
        }
        return $payload;
    }
    /**
    ...
    {
        if (\version_compare(\PHP_VERSION, '5.4.0', '>=') && !(\defined('WPStaging\\Vendor\\JSON_C_VERSION') && \PHP_INT_SIZE > 4)) {
    ...
    private static function getKeyMaterialAndAlgorithm($keyOrKeyArray, $kid = null)
    {
        ...
        if ($keyOrKeyArray instanceof \WPStaging\Vendor\Firebase\JWT\Key) {
        ...
        if (\is_array($keyOrKeyArray) || $keyOrKeyArray instanceof \ArrayAccess) {
            ...
            if ($key instanceof \WPStaging\Vendor\Firebase\JWT\Key) {
