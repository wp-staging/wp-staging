...
class ActionScheduler_ActionFactory
{
     ...
     */
    public function get_stored_action($status, $hook, array $args = array(), \WPStaging\Vendor\ActionScheduler_Schedule $schedule = null, $group = '')
    {
        switch ($status) {
            case \WPStaging\Vendor\ActionScheduler_Store::STATUS_PENDING:
                $action_class = 'ActionScheduler_Action';
                break;
            case \WPStaging\Vendor\ActionScheduler_Store::STATUS_CANCELED:
                $action_class = 'ActionScheduler_CanceledAction';
                if (!\is_null($schedule) && !\is_a($schedule, 'WPStaging\\Vendor\\ActionScheduler_CanceledSchedule') && !\is_a($schedule, 'WPStaging\\Vendor\\ActionScheduler_NullSchedule')) {
                    $schedule = new \WPStaging\Vendor\ActionScheduler_CanceledSchedule($schedule->get_date());
                }
                break;
            default:
        ...
        $action_class = \WPStaging\Vendor\apply_filters('action_scheduler_stored_action_class', $action_class, $status, $hook, $args, $schedule, $group);
         ...
         */
        return \WPStaging\Vendor\apply_filters('action_scheduler_stored_action_instance', $action, $hook, $args, $schedule, $group);
    }
    /**
    ...
    public function async($hook, $args = array(), $group = '')
    {
        ...
        $action = new \WPStaging\Vendor\ActionScheduler_Action($hook, $args, $schedule, $group);
    ...
    {
        $date = \WPStaging\Vendor\as_get_datetime_object($when);
        $schedule = new \WPStaging\Vendor\ActionScheduler_SimpleSchedule($date);
        $action = new \WPStaging\Vendor\ActionScheduler_Action($hook, $args, $schedule, $group);
        return $this->store($action);
    }
    /**
    ...
    public function recurring($hook, $args = array(), $first = null, $interval = null, $group = '')
    {
        ...
        $date = \WPStaging\Vendor\as_get_datetime_object($first);
        $schedule = new \WPStaging\Vendor\ActionScheduler_IntervalSchedule($date, $interval);
        $action = new \WPStaging\Vendor\ActionScheduler_Action($hook, $args, $schedule, $group);
        return $this->store($action);
    }
    /**
    ...
    public function cron($hook, $args = array(), $base_timestamp = null, $schedule = null, $group = '')
    {
        ...
        $date = \WPStaging\Vendor\as_get_datetime_object($base_timestamp);
        $cron = \WPStaging\Vendor\CronExpression::factory($schedule);
        $schedule = new \WPStaging\Vendor\ActionScheduler_CronSchedule($date, $cron);
        $action = new \WPStaging\Vendor\ActionScheduler_Action($hook, $args, $schedule, $group);
        return $this->store($action);
    }
    /**
    ...
    public function repeat($action)
    {
        ...
        $next = $schedule->get_next(\WPStaging\Vendor\as_get_datetime_object());
        if (\is_null($next) || !$schedule->is_recurring()) {
            throw new \InvalidArgumentException(\WPStaging\Vendor\__('Invalid action - must be a recurring action.', 'action-scheduler'));
        }
        $schedule_class = \get_class($schedule);
        $new_schedule = new $schedule($next, $schedule->get_recurrence(), $schedule->get_first_date());
        $new_action = new \WPStaging\Vendor\ActionScheduler_Action($action->get_hook(), $action->get_args(), $new_schedule, $action->get_group());
     ...
     * @return int The ID of the stored action
     */
    protected function store(\WPStaging\Vendor\ActionScheduler_Action $action)
    {
        $store = \WPStaging\Vendor\ActionScheduler_Store::instance();
