...
class ActionScheduler_DataController
{
    ...
    public static function dependencies_met()
    {
        ...
        return $php_support && \WPStaging\Vendor\apply_filters('action_scheduler_migration_dependencies_met', \true);
    }
    /**
    ...
    {
        return \WPStaging\Vendor\get_option(self::STATUS_FLAG) === self::STATUS_COMPLETE;
    }
    /**
    ...
    {
        \WPStaging\Vendor\update_option(self::STATUS_FLAG, self::STATUS_COMPLETE);
    }
    /**
    ...
    {
        \WPStaging\Vendor\delete_option(self::STATUS_FLAG);
    }
    /**
    ...
    public static function free_memory()
    {
        if (0 < self::$sleep_time) {
            ...
            \WPStaging\Vendor\WP_CLI::warning(\sprintf(\WPStaging\Vendor\_n('Stopped the insanity for %d second', 'Stopped the insanity for %d seconds', self::$sleep_time, 'action-scheduler'), self::$sleep_time));
            \sleep(self::$sleep_time);
        }
        \WPStaging\Vendor\WP_CLI::warning(\WPStaging\Vendor\__('Attempting to reduce used memory...', 'action-scheduler'));
        /**
         * @var $wpdb            \wpdb
        ...
        $wpdb->queries = array();
        if (!\is_a($wp_object_cache, 'WPStaging\\Vendor\\WP_Object_Cache')) {
            return;
        }
    ...
    public static function init()
    {
        if (self::is_migration_complete()) {
            \WPStaging\Vendor\add_filter('action_scheduler_store_class', array('ActionScheduler_DataController', 'set_store_class'), 100);
            \WPStaging\Vendor\add_filter('action_scheduler_logger_class', array('ActionScheduler_DataController', 'set_logger_class'), 100);
            \WPStaging\Vendor\add_action('deactivate_plugin', array('ActionScheduler_DataController', 'mark_migration_incomplete'));
        } elseif (self::dependencies_met()) {
            \WPStaging\Vendor\Action_Scheduler\Migration\Controller::init();
        }
        \WPStaging\Vendor\add_action('action_scheduler/progress_tick', array('ActionScheduler_DataController', 'maybe_free_memory'));
    }
    /**
     * Singleton factory.
     */
