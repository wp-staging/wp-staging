 ...
 * @since 3.0.0
 */
class ActionScheduler_DBLogger extends \WPStaging\Vendor\ActionScheduler_Logger
{
    /**
    ...
    public function log($action_id, $message, \DateTime $date = null)
    {
        ...
        /** @var \wpdb $wpdb */
        global $wpdb;
        $wpdb->insert($wpdb->actionscheduler_logs, ['action_id' => $action_id, 'message' => $message, 'log_date_gmt' => $date_gmt, 'log_date_local' => $date_local], ['%d', '%s', '%s', '%s']);
    ...
    private function create_entry_from_db_record($record)
    {
        ...
        $date = \WPStaging\Vendor\as_get_datetime_object($record->log_date_gmt);
        return new \WPStaging\Vendor\ActionScheduler_LogEntry($record->action_id, $record->message, $date);
    }
    /**
    ...
    {
        $table_maker = new \WPStaging\Vendor\ActionScheduler_LoggerSchema();
        $table_maker->register_tables();
        parent::init();
        \WPStaging\Vendor\add_action('action_scheduler_deleted_action', [$this, 'clear_deleted_action_logs'], 10, 1);
    }
    /**
    ...
    public function bulk_log_cancel_actions($action_ids)
    {
        ...
        \WPStaging\Vendor\ActionScheduler_TimezoneHelper::set_local_timezone($date);
        $date_local = $date->format('Y-m-d H:i:s');
        $message = \WPStaging\Vendor\__('action canceled', 'action-scheduler');
