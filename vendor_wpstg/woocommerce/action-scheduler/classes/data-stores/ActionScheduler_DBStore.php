 ...
 * @since 3.0.0
 */
class ActionScheduler_DBStore extends \WPStaging\Vendor\ActionScheduler_Store
{
    /** @var int */
    ...
    {
        $table_maker = new \WPStaging\Vendor\ActionScheduler_StoreSchema();
     ...
     */
    public function save_action(\WPStaging\Vendor\ActionScheduler_Action $action, \DateTime $date = null)
    {
        try {
            ...
            $args = \WPStaging\Vendor\wp_json_encode($action->get_args());
            ...
            if (\WPStaging\Vendor\is_wp_error($action_id)) {
                throw new \RuntimeException($action_id->get_error_message());
            } elseif (empty($action_id)) {
                throw new \RuntimeException($wpdb->last_error ? $wpdb->last_error : \WPStaging\Vendor\__('Database error.', 'action-scheduler'));
            }
            \WPStaging\Vendor\do_action('action_scheduler_stored_action', $action_id);
            return $action_id;
        } catch (\Exception $e) {
    ...
    {
        $encoded = \WPStaging\Vendor\wp_json_encode($args);
    ...
    public function fetch_action($action_id)
    {
        ...
        try {
            ...
            \WPStaging\Vendor\do_action('action_scheduler_failed_fetch_action', $action_id, $exception);
            return $this->get_null_action();
        }
        return $action;
    }
    ...
    {
        $hook = $data->hook;
        $args = \json_decode($data->args, \true);
        $schedule = \unserialize($data->schedule);
        $this->validate_args($args, $data->action_id);
        $this->validate_schedule($schedule, $data->action_id);
        if (empty($schedule)) {
            $schedule = new \WPStaging\Vendor\ActionScheduler_NullSchedule();
        }
        $group = $data->group ? $data->group : '';
        return \WPStaging\Vendor\ActionScheduler::factory()->get_stored_action($data->status, $data->hook, $args, $schedule, $group);
    }
    /**
     * Find an action.
     *
    ...
    {
        $params = \WPStaging\Vendor\wp_parse_args($params, ['args' => null, 'status' => self::STATUS_PENDING, 'group' => '']);
        /** @var wpdb $wpdb */
        global $wpdb;
    ...
    protected function get_query_actions_sql(array $query, $select_or_count = 'select')
    {
        if (!\in_array($select_or_count, array('select', 'count'))) {
            throw new \InvalidArgumentException(\WPStaging\Vendor\__('Invalid value for select or count parameter. Cannot query actions.', 'action-scheduler'));
        }
        $query = \WPStaging\Vendor\wp_parse_args($query, ['hook' => '', 'args' => null, 'date' => null, 'date_compare' => '<=', 'modified' => null, 'modified_compare' => '<=', 'group' => '', 'status' => '', 'claimed' => null, 'per_page' => 5, 'offset' => 0, 'orderby' => 'date', 'order' => 'ASC']);
        /** @var \wpdb $wpdb */
        global $wpdb;
    ...
    protected function bulk_cancel_actions($query_args)
    {
        ...
        $action_ids = \true;
        $query_args = \WPStaging\Vendor\wp_parse_args($query_args, ['per_page' => 1000, 'status' => self::STATUS_PENDING]);
        while ($action_ids) {
    ...
    public function get_date($action_id)
    {
        ...
        \WPStaging\Vendor\ActionScheduler_TimezoneHelper::set_local_timezone($date);
        return $date;
    }
    /**
    ...
    protected function get_date_gmt($action_id)
    {
        ...
        if ($record->status == self::STATUS_PENDING) {
            return \WPStaging\Vendor\as_get_datetime_object($record->scheduled_date_gmt);
        } else {
            return \WPStaging\Vendor\as_get_datetime_object($record->last_attempt_gmt);
        }
    }
    /**
    ...
    protected function generate_claim_id()
    {
        /** @var \wpdb $wpdb */
        global $wpdb;
        $now = \WPStaging\Vendor\as_get_datetime_object();
    ...
    protected function claim_actions($claim_id, $limit, \DateTime $before_date = null, $hooks = array(), $group = '')
    {
        ...
        $params = array($claim_id, $now->format('Y-m-d H:i:s'), \WPStaging\Vendor\current_time('mysql'));
    ...
    public function log_execution($action_id)
    {
        ...
        $sql = $wpdb->prepare($sql, self::STATUS_RUNNING, \WPStaging\Vendor\current_time('mysql', \true), \WPStaging\Vendor\current_time('mysql'), $action_id);
        $wpdb->query($sql);
    }
    /**
    ...
    public function mark_complete($action_id)
    {
        /** @var \wpdb $wpdb */
        global $wpdb;
        $updated = $wpdb->update($wpdb->actionscheduler_actions, ['status' => self::STATUS_COMPLETE, 'last_attempt_gmt' => \WPStaging\Vendor\current_time('mysql', \true), 'last_attempt_local' => \WPStaging\Vendor\current_time('mysql')], ['action_id' => $action_id], ['%s'], ['%d']);
        if (empty($updated)) {
            throw new \InvalidArgumentException(\sprintf(\WPStaging\Vendor\__('Unidentified action %s', 'action-scheduler'), $action_id));
        }
    }
    ...
    public function get_status($action_id)
    {
        ...
        if ($status === null) {
            ...
            throw new \RuntimeException(\WPStaging\Vendor\__('Unknown status found for action.', 'action-scheduler'));
        } else {
            return $status;
        }
    }
