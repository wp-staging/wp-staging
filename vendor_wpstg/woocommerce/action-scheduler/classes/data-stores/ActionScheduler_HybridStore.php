...
use WPStaging\Vendor\ActionScheduler_Store as Store;
 ...
 * @since 3.0.0
 */
class ActionScheduler_HybridStore extends \WPStaging\Vendor\ActionScheduler_Store
{
     ...
     */
    public function __construct(\WPStaging\Vendor\Action_Scheduler\Migration\Config $config = null)
    {
        $this->demarkation_id = (int) \WPStaging\Vendor\get_option(self::DEMARKATION_OPTION, 0);
        if (empty($config)) {
        ...
        $this->migration_runner = new \WPStaging\Vendor\Action_Scheduler\Migration\Runner($config);
    }
    /**
    ...
    {
        \WPStaging\Vendor\add_action('action_scheduler/created_table', [$this, 'set_autoincrement'], 10, 2);
        $this->primary_store->init();
        $this->secondary_store->init();
        \WPStaging\Vendor\remove_action('action_scheduler/created_table', [$this, 'set_autoincrement'], 10);
    }
    /**
    ...
    {
        if (\WPStaging\Vendor\ActionScheduler_StoreSchema::ACTIONS_TABLE === $table_suffix) {
            ...
            $null_action = new \WPStaging\Vendor\ActionScheduler_NullAction();
            ...
            $row_count = $wpdb->insert($wpdb->{\WPStaging\Vendor\ActionScheduler_StoreSchema::ACTIONS_TABLE}, ['action_id' => $this->demarkation_id, 'hook' => '', 'status' => '', 'scheduled_date_gmt' => $date_gmt, 'scheduled_date_local' => $date_local, 'last_attempt_gmt' => $date_gmt, 'last_attempt_local' => $date_local]);
            if ($row_count > 0) {
                $wpdb->delete($wpdb->{\WPStaging\Vendor\ActionScheduler_StoreSchema::ACTIONS_TABLE}, ['action_id' => $this->demarkation_id]);
            }
        }
    }
    /**
    ...
    private function set_demarkation_id($id = null)
    {
        ...
        \WPStaging\Vendor\update_option(self::DEMARKATION_OPTION, $id);
        return $id;
    }
    /**
    ...
    {
        $store = $this->get_store_from_action_id($action_id, \true);
        if ($store) {
    ...
    {
        if ($primary_first) {
            $stores = [$this->primary_store, $this->secondary_store];
        } elseif ($action_id < $this->demarkation_id) {
            $stores = [$this->secondary_store, $this->primary_store];
        } else {
            $stores = [$this->primary_store];
        }
