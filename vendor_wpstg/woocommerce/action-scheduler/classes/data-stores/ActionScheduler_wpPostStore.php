 ...
 */
class ActionScheduler_wpPostStore extends \WPStaging\Vendor\ActionScheduler_Store
{
    ...
    /** @var DateTimeZone */
    protected $local_timezone = \WPStaging\Vendor\NULL;
    public function save_action(\WPStaging\Vendor\ActionScheduler_Action $action, \DateTime $scheduled_date = \WPStaging\Vendor\NULL)
    {
        try {
            ...
            \WPStaging\Vendor\do_action('action_scheduler_stored_action', $post_id);
            return $post_id;
        } catch (\Exception $e) {
            throw new \RuntimeException(\sprintf(\WPStaging\Vendor\__('Error saving action: %s', 'action-scheduler'), $e->getMessage()), 0);
        }
    }
    protected function create_post_array(\WPStaging\Vendor\ActionScheduler_Action $action, \DateTime $scheduled_date = \WPStaging\Vendor\NULL)
    {
    ...
    {
        \WPStaging\Vendor\add_filter('wp_insert_post_data', array($this, 'filter_insert_post_data'), 10, 1);
        \WPStaging\Vendor\add_filter('pre_wp_unique_post_slug', array($this, 'set_unique_post_slug'), 10, 5);
        $has_kses = \false !== \WPStaging\Vendor\has_filter('content_save_pre', 'wp_filter_post_kses');
        if ($has_kses) {
            // Prevent KSES from corrupting JSON in post_content.
            \WPStaging\Vendor\kses_remove_filters();
        }
        $post_id = \WPStaging\Vendor\wp_insert_post($post_array);
        if ($has_kses) {
            \WPStaging\Vendor\kses_init_filters();
        }
        \WPStaging\Vendor\remove_filter('wp_insert_post_data', array($this, 'filter_insert_post_data'), 10);
        \WPStaging\Vendor\remove_filter('pre_wp_unique_post_slug', array($this, 'set_unique_post_slug'), 10);
        if (\WPStaging\Vendor\is_wp_error($post_id) || empty($post_id)) {
            throw new \RuntimeException(\WPStaging\Vendor\__('Unable to save action.', 'action-scheduler'));
        }
        return $post_id;
    }
    ...
    public function set_unique_post_slug($override_slug, $slug, $post_ID, $post_status, $post_type)
    {
        if (self::POST_TYPE == $post_type) {
            $override_slug = \uniqid(self::POST_TYPE . '-', \true) . '-' . \WPStaging\Vendor\wp_generate_password(32, \false);
        }
        return $override_slug;
    }
    protected function save_post_schedule($post_id, $schedule)
    {
        \WPStaging\Vendor\update_post_meta($post_id, self::SCHEDULE_META_KEY, $schedule);
    }
    protected function save_action_group($post_id, $group)
    {
        if (empty($group)) {
            \WPStaging\Vendor\wp_set_object_terms($post_id, array(), self::GROUP_TAXONOMY, \WPStaging\Vendor\FALSE);
        } else {
            \WPStaging\Vendor\wp_set_object_terms($post_id, array($group), self::GROUP_TAXONOMY, \WPStaging\Vendor\FALSE);
        }
    }
    public function fetch_action($action_id)
    {
        ...
        } catch (\WPStaging\Vendor\ActionScheduler_InvalidActionException $exception) {
            \WPStaging\Vendor\do_action('action_scheduler_failed_fetch_action', $post->ID, $exception);
            return $this->get_null_action();
        }
        return $action;
    }
    ...
    {
        if (empty($action_id)) {
            return \WPStaging\Vendor\NULL;
        }
        return \WPStaging\Vendor\get_post($action_id);
    }
    protected function get_null_action()
    {
        return new \WPStaging\Vendor\ActionScheduler_NullAction();
    }
    protected function make_action_from_post($post)
    {
        $hook = $post->post_title;
        $args = \json_decode($post->post_content, \true);
        $this->validate_args($args, $post->ID);
        $schedule = \WPStaging\Vendor\get_post_meta($post->ID, self::SCHEDULE_META_KEY, \true);
        $this->validate_schedule($schedule, $post->ID);
        $group = \WPStaging\Vendor\wp_get_object_terms($post->ID, self::GROUP_TAXONOMY, array('fields' => 'names'));
        $group = empty($group) ? '' : \reset($group);
        return \WPStaging\Vendor\ActionScheduler::factory()->get_stored_action($this->get_action_status_by_post_status($post->post_status), $hook, $args, $schedule, $group);
    }
    /**
    ...
    {
        $params = \WPStaging\Vendor\wp_parse_args($params, array('args' => \WPStaging\Vendor\NULL, 'status' => \WPStaging\Vendor\ActionScheduler_Store::STATUS_PENDING, 'group' => ''));
        /** @var wpdb $wpdb */
        global $wpdb;
    ...
    protected function get_query_actions_sql(array $query, $select_or_count = 'select')
    {
        if (!\in_array($select_or_count, array('select', 'count'))) {
            throw new \InvalidArgumentException(\WPStaging\Vendor\__('Invalid schedule. Cannot save action.', 'action-scheduler'));
        }
        $query = \WPStaging\Vendor\wp_parse_args($query, array('hook' => '', 'args' => \WPStaging\Vendor\NULL, 'date' => \WPStaging\Vendor\NULL, 'date_compare' => '<=', 'modified' => \WPStaging\Vendor\NULL, 'modified_compare' => '<=', 'group' => '', 'status' => '', 'claimed' => \WPStaging\Vendor\NULL, 'per_page' => 5, 'offset' => 0, 'orderby' => 'date', 'order' => 'ASC', 'search' => ''));
        /** @var wpdb $wpdb */
        global $wpdb;
        ...
        if ($query['claimed'] === \WPStaging\Vendor\TRUE) {
            $sql .= " AND p.post_password != ''";
        } elseif ($query['claimed'] === \WPStaging\Vendor\FALSE) {
            $sql .= " AND p.post_password = ''";
    ...
    public function action_counts()
    {
        ...
        $posts_count_by_status = (array) \WPStaging\Vendor\wp_count_posts(self::POST_TYPE, 'readable');
    ...
    {
        $post = \WPStaging\Vendor\get_post($action_id);
        if (empty($post) || $post->post_type != self::POST_TYPE) {
            throw new \InvalidArgumentException(\sprintf(\WPStaging\Vendor\__('Unidentified action %s', 'action-scheduler'), $action_id));
        }
        \WPStaging\Vendor\do_action('action_scheduler_canceled_action', $action_id);
        \WPStaging\Vendor\add_filter('pre_wp_unique_post_slug', array($this, 'set_unique_post_slug'), 10, 5);
        \WPStaging\Vendor\wp_trash_post($action_id);
        \WPStaging\Vendor\remove_filter('pre_wp_unique_post_slug', array($this, 'set_unique_post_slug'), 10);
    }
    public function delete_action($action_id)
    {
        ...
        \WPStaging\Vendor\do_action('action_scheduler_deleted_action', $action_id);
        \WPStaging\Vendor\wp_delete_post($action_id, \WPStaging\Vendor\TRUE);
    }
    /**
     * @param string $action_id
     *
    ...
    public function get_date($action_id)
    {
        ...
        return \WPStaging\Vendor\ActionScheduler_TimezoneHelper::set_local_timezone($next);
    }
    /**
     * @param string $action_id
     *
    ...
    public function get_date_gmt($action_id)
    {
        ...
        if ($post->post_status == 'publish') {
            return \WPStaging\Vendor\as_get_datetime_object($post->post_modified_gmt);
        } else {
            return \WPStaging\Vendor\as_get_datetime_object($post->post_date_gmt);
        }
    }
    /**
    ...
    public function stake_claim($max_actions = 10, \DateTime $before_date = null, $hooks = array(), $group = '')
    {
        ...
        return new \WPStaging\Vendor\ActionScheduler_ActionClaim($claim_id, $action_ids);
    }
    /**
     * @return int
     */
    ...
    {
        $claim_id = \md5(\microtime(\true) . \rand(0, 1000));
    ...
    protected function claim_actions($claim_id, $limit, \DateTime $before_date = null, $hooks = array(), $group = '')
    {
        ...
        $date = null === $before_date ? \WPStaging\Vendor\as_get_datetime_object() : clone $before_date;
        ...
        $params = array($claim_id, \WPStaging\Vendor\current_time('mysql', \true), \WPStaging\Vendor\current_time('mysql'));
        // Build initial WHERE clause.
        $where = "WHERE post_type = %s AND post_status = %s AND post_password = ''";
        $params[] = self::POST_TYPE;
        $params[] = \WPStaging\Vendor\ActionScheduler_Store::STATUS_PENDING;
        if (!empty($hooks)) {
        ...
        if ($rows_affected === \false) {
            throw new \RuntimeException(\WPStaging\Vendor\__('Unable to claim actions. Database error.', 'action-scheduler'));
        }
    ...
    protected function get_actions_by_group($group, $limit, \DateTime $date)
    {
        ...
        if (!\WPStaging\Vendor\term_exists($group, self::GROUP_TAXONOMY)) {
            throw new \InvalidArgumentException(\sprintf(\WPStaging\Vendor\__('The group "%s" does not exist.', 'action-scheduler'), $group));
        }
        // Set up a query for post IDs to use later.
        $query = new \WPStaging\Vendor\WP_Query();
        $query_args = array('fields' => 'ids', 'post_type' => self::POST_TYPE, 'post_status' => \WPStaging\Vendor\ActionScheduler_Store::STATUS_PENDING, 'has_password' => \false, 'posts_per_page' => $limit * 3, 'suppress_filters' => \true, 'no_found_rows' => \true, 'orderby' => array('menu_order' => 'ASC', 'date' => 'ASC', 'ID' => 'ASC'), 'date_query' => array('column' => 'post_date_gmt', 'before' => $date->format('Y-m-d H:i'), 'inclusive' => \true), 'tax_query' => array(array('taxonomy' => self::GROUP_TAXONOMY, 'field' => 'slug', 'terms' => $group, 'include_children' => \false)));
    ...
    public function release_claim(\WPStaging\Vendor\ActionScheduler_ActionClaim $claim)
    {
        ...
        if ($result === \false) {
            ...
            throw new \RuntimeException(\sprintf(\WPStaging\Vendor\__('Unable to unlock claim %s. Database error.', 'action-scheduler'), $claim->get_id()));
        }
    }
    /**
     * @param string $action_id
    ...
    public function unclaim_action($action_id)
    {
        ...
        if ($result === \false) {
            ...
            throw new \RuntimeException(\sprintf(\WPStaging\Vendor\__('Unable to unlock claim on action %s. Database error.', 'action-scheduler'), $action_id));
        }
    }
    public function mark_failure($action_id)
    {
        ...
        if ($result === \false) {
            /* translators: %s: action ID */
            throw new \RuntimeException(\sprintf(\WPStaging\Vendor\__('Unable to mark failure on action %s. Database error.', 'action-scheduler'), $action_id));
        }
    }
    /**
    ...
    public function get_status($action_id)
    {
        ...
        if ($status === null) {
            throw new \InvalidArgumentException(\WPStaging\Vendor\__('Invalid action ID. No status found.', 'action-scheduler'));
        }
    ...
    public function log_execution($action_id)
    {
        ...
        $sql = $wpdb->prepare($sql, self::STATUS_RUNNING, \WPStaging\Vendor\current_time('mysql', \true), \WPStaging\Vendor\current_time('mysql'), $action_id, self::POST_TYPE);
        $wpdb->query($sql);
    }
    /**
    ...
    public function mark_complete($action_id)
    {
        ...
        \WPStaging\Vendor\add_filter('wp_insert_post_data', array($this, 'filter_insert_post_data'), 10, 1);
        \WPStaging\Vendor\add_filter('pre_wp_unique_post_slug', array($this, 'set_unique_post_slug'), 10, 5);
        $result = \WPStaging\Vendor\wp_update_post(array('ID' => $action_id, 'post_status' => 'publish'), \WPStaging\Vendor\TRUE);
        \WPStaging\Vendor\remove_filter('wp_insert_post_data', array($this, 'filter_insert_post_data'), 10);
        \WPStaging\Vendor\remove_filter('pre_wp_unique_post_slug', array($this, 'set_unique_post_slug'), 10);
        if (\WPStaging\Vendor\is_wp_error($result)) {
    ...
    {
        \WPStaging\Vendor\wp_update_post(array('ID' => $action_id, 'post_status' => 'migrated'));
    }
    /**
    ...
    {
        global $wpdb;
        $dependencies_met = \WPStaging\Vendor\get_transient(self::DEPENDENCIES_MET);
        if (empty($dependencies_met)) {
            $maximum_args_length = \WPStaging\Vendor\apply_filters('action_scheduler_maximum_args_length', 191);
            ...
            \WPStaging\Vendor\set_transient(self::DEPENDENCIES_MET, $dependencies_met, \WPStaging\Vendor\DAY_IN_SECONDS);
        }
    ...
    protected function validate_action(\WPStaging\Vendor\ActionScheduler_Action $action)
    {
        try {
            ...
            $message = \sprintf(\WPStaging\Vendor\__('%s Support for strings longer than this will be removed in a future version.', 'action-scheduler'), $e->getMessage());
            \WPStaging\Vendor\_doing_it_wrong('ActionScheduler_Action::$args', $message, '2.1.0');
        }
    }
    /**
     * @codeCoverageIgnore
    ...
    {
        \WPStaging\Vendor\add_filter('action_scheduler_migration_dependencies_met', array($this, 'migration_dependencies_met'));
        $post_type_registrar = new \WPStaging\Vendor\ActionScheduler_wpPostStore_PostTypeRegistrar();
        $post_type_registrar->register();
        $post_status_registrar = new \WPStaging\Vendor\ActionScheduler_wpPostStore_PostStatusRegistrar();
        $post_status_registrar->register();
        $taxonomy_registrar = new \WPStaging\Vendor\ActionScheduler_wpPostStore_TaxonomyRegistrar();
