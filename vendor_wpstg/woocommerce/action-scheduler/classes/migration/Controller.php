...
namespace WPStaging\Vendor\Action_Scheduler\Migration;
...
class Controller
{
     ...
     */
    protected function __construct(\WPStaging\Vendor\Action_Scheduler\Migration\Scheduler $migration_scheduler)
    {
    ...
    {
        if (\WPStaging\Vendor\ActionScheduler_DataController::is_migration_complete()) {
            return \WPStaging\Vendor\ActionScheduler_DataController::DATASTORE_CLASS;
        } elseif (\WPStaging\Vendor\ActionScheduler_Store::DEFAULT_CLASS !== $class) {
    ...
    {
        \WPStaging\Vendor\ActionScheduler_Store::instance();
        if ($this->has_custom_datastore()) {
            $this->logger_classname = $class;
            return $class;
        } else {
            return \WPStaging\Vendor\ActionScheduler_DataController::LOGGER_CLASS;
        }
    }
    /**
    ...
    {
        if (\WPStaging\Vendor\ActionScheduler_DataController::is_migration_complete() || $this->migration_scheduler->is_migration_scheduled()) {
            return;
        }
    ...
    public function get_migration_config_object()
    {
        static $config = null;
        if (!$config) {
            $source_store = $this->store_classname ? new $this->store_classname() : new \WPStaging\Vendor\ActionScheduler_wpPostStore();
            $source_logger = $this->logger_classname ? new $this->logger_classname() : new \WPStaging\Vendor\ActionScheduler_wpCommentLogger();
            $config = new \WPStaging\Vendor\Action_Scheduler\Migration\Config();
            $config->set_source_store($source_store);
            $config->set_source_logger($source_logger);
            $config->set_destination_store(new \WPStaging\Vendor\ActionScheduler_DBStoreMigrator());
            $config->set_destination_logger(new \WPStaging\Vendor\ActionScheduler_DBLogger());
            if (\defined('WPStaging\\Vendor\\WP_CLI') && WP_CLI) {
                $config->set_progress_bar(new \WPStaging\Vendor\Action_Scheduler\WP_CLI\ProgressBar('', 0));
            }
        }
    ...
    {
        if (!$this->allow_migration() || \WPStaging\Vendor\ActionScheduler_DataController::is_migration_complete()) {
            return;
        }
    ...
    {
        if (!\WPStaging\Vendor\ActionScheduler_DataController::dependencies_met()) {
            return \false;
        }
    ...
    public static function instance()
    {
        if (!isset(self::$instance)) {
            self::$instance = new static(new \WPStaging\Vendor\Action_Scheduler\Migration\Scheduler());
        }
