...
class Runner
{
     ...
     */
    public function __construct(\WPStaging\Vendor\Action_Scheduler\Migration\Config $config)
    {
        ...
        $this->batch_fetcher = new \WPStaging\Vendor\Action_Scheduler\Migration\BatchFetcher($this->source_store);
        if ($config->get_dry_run()) {
            $this->log_migrator = new \WPStaging\Vendor\Action_Scheduler\Migration\DryRun_LogMigrator($this->source_logger, $this->destination_logger);
            $this->action_migrator = new \WPStaging\Vendor\Action_Scheduler\Migration\DryRun_ActionMigrator($this->source_store, $this->destination_store, $this->log_migrator);
        } else {
            $this->log_migrator = new \WPStaging\Vendor\Action_Scheduler\Migration\LogMigrator($this->source_logger, $this->destination_logger);
            $this->action_migrator = new \WPStaging\Vendor\Action_Scheduler\Migration\ActionMigrator($this->source_store, $this->destination_store, $this->log_migrator);
        }
    ...
    public function migrate_actions(array $action_ids)
    {
        ...
        \WPStaging\Vendor\ActionScheduler::logger()->unhook_stored_action();
        ...
        \WPStaging\Vendor\ActionScheduler::logger()->hook_stored_action();
