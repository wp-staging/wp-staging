...
abstract class ActionScheduler_Abstract_Schema
{
    ...
    private function schema_update_required()
    {
        ...
        $version_found_in_db = \WPStaging\Vendor\get_option($option_name, 0);
        ...
        if (0 === $version_found_in_db) {
            ...
            $version_found_in_db = \WPStaging\Vendor\get_option($plugin_option_name, 0);
            \WPStaging\Vendor\delete_option($plugin_option_name);
        }
    ...
    private function mark_schema_update_complete()
    {
        ...
        $value_to_save = (string) $this->schema_version . '.0.' . \time();
        \WPStaging\Vendor\update_option($option_name, $value_to_save);
    }
    /**
    ...
    {
        require_once \WPStaging\Vendor\ABSPATH . 'wp-admin/includes/upgrade.php';
        $definition = $this->get_table_definition($table);
        if ($definition) {
            $updated = \WPStaging\Vendor\dbDelta($definition);
            foreach ($updated as $updated_table => $update_description) {
                if (\strpos($update_description, 'Created table') === 0) {
                    \WPStaging\Vendor\do_action('action_scheduler/created_table', $updated_table, $table);
                }
            }
        }
    }
