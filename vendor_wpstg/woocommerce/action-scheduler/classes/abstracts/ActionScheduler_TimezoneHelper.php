...
{
    private static $local_timezone = \WPStaging\Vendor\NULL;
    /**
    ...
    public static function set_local_timezone(\DateTime $date)
    {
        ...
        if (!\is_a($date, 'WPStaging\\Vendor\\ActionScheduler_DateTime')) {
            $date = \WPStaging\Vendor\as_get_datetime_object($date->format('U'));
        }
        if (\WPStaging\Vendor\get_option('timezone_string')) {
    ...
    protected static function get_local_timezone_string($reset = \false)
    {
        ...
        $timezone = \WPStaging\Vendor\get_option('timezone_string');
        if ($timezone) {
            return $timezone;
        }
        // Get UTC offset, if it isn't set then return UTC.
        $utc_offset = \intval(\WPStaging\Vendor\get_option('gmt_offset', 0));
        if (0 === $utc_offset) {
            return 'UTC';
        }
        ...
        $timezone = \timezone_name_from_abbr('', $utc_offset);
        if ($timezone) {
            return $timezone;
        }
    ...
    protected static function get_local_timezone_offset()
    {
        ...
        if ($timezone) {
            ...
            return \floatval(\WPStaging\Vendor\get_option('gmt_offset', 0)) * \WPStaging\Vendor\HOUR_IN_SECONDS;
        }
    }
    /**
     * @deprecated 2.1.0
     */
    public static function get_local_timezone($reset = \WPStaging\Vendor\FALSE)
    {
        \WPStaging\Vendor\_deprecated_function(__FUNCTION__, '2.1.0', 'ActionScheduler_TimezoneHelper::set_local_timezone()');
        if ($reset) {
            self::$local_timezone = \WPStaging\Vendor\NULL;
        }
        if (!isset(self::$local_timezone)) {
            $tzstring = \WPStaging\Vendor\get_option('timezone_string');
            if (empty($tzstring)) {
                $gmt_offset = \WPStaging\Vendor\get_option('gmt_offset');
                if ($gmt_offset == 0) {
                    $tzstring = 'UTC';
                } else {
                    $gmt_offset *= \WPStaging\Vendor\HOUR_IN_SECONDS;
                    $tzstring = \timezone_name_from_abbr('', $gmt_offset, 1);
                    // If there's no timezone string, try again with no DST.
                    if (\false === $tzstring) {
                        $tzstring = \timezone_name_from_abbr('', $gmt_offset, 0);
                    }
