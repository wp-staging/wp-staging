...
if (!\class_exists('WPStaging\\Vendor\\WP_List_Table')) {
    require_once \WPStaging\Vendor\ABSPATH . 'wp-admin/includes/class-wp-list-table.php';
}
/**
 ...
 * @since  2.0.0
 */
abstract class ActionScheduler_Abstract_ListTable extends \WPStaging\Vendor\WP_List_Table
{
    /**
     * The table name
     */
    ...
    protected function process_bulk_action()
    {
        ...
        \WPStaging\Vendor\check_admin_referer('bulk-' . $this->_args['plural']);
        $method = 'bulk_' . $action;
        ...
        \WPStaging\Vendor\wp_redirect(\WPStaging\Vendor\remove_query_arg(array('_wp_http_referer', '_wpnonce', 'ID', 'action', 'action2'), \WPStaging\Vendor\wp_unslash($_SERVER['REQUEST_URI'])));
        exit;
    }
    /**
    ...
    {
        $store = \WPStaging\Vendor\ActionScheduler::store();
    ...
    public function get_sortable_columns()
    {
        ...
        foreach ($this->sort_by as $column) {
            $sort_by[$column] = array($column, \true);
        }
        return $sort_by;
    }
    /**
    ...
    {
        $columns = \array_merge(array('cb' => '<input type="checkbox" />'), $this->columns);
        return $columns;
    }
    /**
    ...
    protected function get_items_query_order()
    {
        ...
        $orderby = \WPStaging\Vendor\esc_sql($this->get_request_orderby());
        $order = \WPStaging\Vendor\esc_sql($this->get_request_order());
    ...
    protected function get_request_orderby()
    {
        ...
        if (!empty($_GET['orderby']) && \in_array($_GET['orderby'], $valid_sortable_columns)) {
            $orderby = \WPStaging\Vendor\sanitize_text_field($_GET['orderby']);
        } else {
    ...
    public function prepare_items()
    {
        ...
        if (!empty($_REQUEST['_wp_http_referer'])) {
            ...
            \WPStaging\Vendor\wp_redirect(\WPStaging\Vendor\remove_query_arg(array('_wp_http_referer', '_wpnonce'), \WPStaging\Vendor\wp_unslash($_SERVER['REQUEST_URI'])));
            exit;
        }
        ...
        $where = \array_filter(array($this->get_items_query_search(), $this->get_items_query_filters()));
        ...
        $this->set_items($wpdb->get_results($sql, \WPStaging\Vendor\ARRAY_A));
    ...
    public function extra_tablenav($which)
    {
        ...
        foreach ($this->filter_by as $id => $options) {
            ...
            echo '<select name="filter_by[' . \WPStaging\Vendor\esc_attr($id) . ']" class="first" id="filter-by-' . \WPStaging\Vendor\esc_attr($id) . '">';
            foreach ($options as $value => $label) {
                echo '<option value="' . \WPStaging\Vendor\esc_attr($value) . '" ' . \WPStaging\Vendor\esc_html($value == $default ? 'selected' : '') . '>' . \WPStaging\Vendor\esc_html($label) . '</option>';
            }
            echo '</select>';
        }
        \WPStaging\Vendor\submit_button(\WPStaging\Vendor\esc_html__('Filter', 'action-scheduler'), '', 'filter_action', \false, array('id' => 'post-query-submit'));
        echo '</div>';
    }
    /**
    ...
    {
        return '<input name="ID[]" type="checkbox" value="' . \WPStaging\Vendor\esc_attr($row[$this->ID]) . '" />';
    }
    /**
    ...
    protected function maybe_render_actions($row, $column_name)
    {
        ...
        foreach ($this->row_actions[$column_name] as $action_key => $action) {
            ...
            $action_link = !empty($action['link']) ? $action['link'] : \WPStaging\Vendor\add_query_arg(array('row_action' => $action_key, 'row_id' => $row_id, 'nonce' => \WPStaging\Vendor\wp_create_nonce($action_key . '::' . $row_id)));
            $span_class = !empty($action['class']) ? $action['class'] : $action_key;
            $separator = $action_count < \count($this->row_actions[$column_name]) ? ' | ' : '';
            $actions .= \sprintf('<span class="%s">', \WPStaging\Vendor\esc_attr($span_class));
            $actions .= \sprintf('<a href="%1$s" title="%2$s">%3$s</a>', \WPStaging\Vendor\esc_url($action_link), \WPStaging\Vendor\esc_attr($action['desc']), \WPStaging\Vendor\esc_html($action['name']));
    ...
    protected function process_row_actions()
    {
        ...
        if ($_REQUEST['nonce'] === \WPStaging\Vendor\wp_create_nonce($_REQUEST['row_action'] . '::' . $_REQUEST['row_id']) && \method_exists($this, $method)) {
            $this->{$method}($_REQUEST['row_id']);
        }
        \WPStaging\Vendor\wp_redirect(\WPStaging\Vendor\remove_query_arg(array('row_id', 'row_action', 'nonce'), \WPStaging\Vendor\wp_unslash($_SERVER['REQUEST_URI'])));
        exit;
    }
    /**
    ...
    {
        $column_html = \WPStaging\Vendor\esc_html($item[$column_name]);
    ...
    {
        echo '<h1 class="wp-heading-inline">' . \WPStaging\Vendor\esc_attr($this->table_header) . '</h1>';
        if ($this->get_request_search_query()) {
            /* translators: %s: search query */
            echo '<span class="subtitle">' . \WPStaging\Vendor\esc_attr(\sprintf(\WPStaging\Vendor\__('Search results for "%s"', 'action-scheduler'), $this->get_request_search_query())) . '</span>';
        }
    ...
    protected function display_admin_notices()
    {
        foreach ($this->admin_notices as $notice) {
            ...
            echo '	<p>' . \WPStaging\Vendor\wp_kses_post($notice['message']) . '</p>';
            echo '</div>';
        }
    }
    /**
    ...
    protected function display_filter_by_status()
    {
        ...
        foreach ($this->status_counts as $status_name => $count) {
            ...
            $status_filter_url = 'all' === $status_name ? \WPStaging\Vendor\remove_query_arg('status') : \WPStaging\Vendor\add_query_arg('status', $status_name);
            $status_filter_url = \WPStaging\Vendor\remove_query_arg(array('paged', 's'), $status_filter_url);
            $status_list_items[] = \sprintf($status_list_item, \WPStaging\Vendor\esc_attr($status_name), \WPStaging\Vendor\esc_url($status_filter_url), \WPStaging\Vendor\esc_html(\ucfirst($status_name)), \WPStaging\Vendor\absint($count));
        }
    ...
    {
        echo '<form id="' . \WPStaging\Vendor\esc_attr($this->_args['plural']) . '-filter" method="get">';
        foreach ($_GET as $key => $value) {
            if ('_' === $key[0] || 'paged' === $key) {
                continue;
            }
            echo '<input type="hidden" name="' . \WPStaging\Vendor\esc_attr($key) . '" value="' . \WPStaging\Vendor\esc_attr($value) . '" />';
        }
    ...
    {
        return \WPStaging\Vendor\esc_html__('Search', 'action-scheduler');
    }
}
