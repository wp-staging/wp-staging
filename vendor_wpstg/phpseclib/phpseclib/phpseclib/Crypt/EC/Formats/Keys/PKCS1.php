...
abstract class PKCS1 extends \WPStaging\Vendor\phpseclib3\Crypt\Common\Formats\Keys\PKCS1
{
    ...
    public static function load($key, $password = '')
    {
        ...
        if (\strpos($key, 'BEGIN EC PARAMETERS') && \strpos($key, 'BEGIN EC PRIVATE KEY')) {
            ...
            $ecPrivate = \WPStaging\Vendor\phpseclib3\File\ASN1::asn1map($decoded[0], \WPStaging\Vendor\phpseclib3\File\ASN1\Maps\ECPrivateKey::MAP);
            ...
            $ecParams = \WPStaging\Vendor\phpseclib3\File\ASN1::asn1map($decoded[0], \WPStaging\Vendor\phpseclib3\File\ASN1\Maps\ECParameters::MAP);
            ...
            $components['dA'] = new \WPStaging\Vendor\phpseclib3\Math\BigInteger($ecPrivate['privateKey'], 256);
        ...
        $key = \WPStaging\Vendor\phpseclib3\File\ASN1::asn1map($decoded[0], \WPStaging\Vendor\phpseclib3\File\ASN1\Maps\ECParameters::MAP);
        if (\is_array($key)) {
     ...
     * @return string
     */
    public static function saveParameters(\WPStaging\Vendor\phpseclib3\Crypt\EC\BaseCurves\Base $curve, array $options = [])
    {
        self::initialize_static_variables();
        ...
        return "-----BEGIN EC PARAMETERS-----\r\n" . \chunk_split(\WPStaging\Vendor\phpseclib3\Common\Functions\Strings::base64_encode($key), 64) . "-----END EC PARAMETERS-----\r\n";
    }
    /**
    ...
    public static function savePrivateKey(\WPStaging\Vendor\phpseclib3\Math\BigInteger $privateKey, \WPStaging\Vendor\phpseclib3\Crypt\EC\BaseCurves\Base $curve, array $publicKey, $secret = null, $password = '', array $options = [])
    {
        self::initialize_static_variables();
        if ($curve instanceof \WPStaging\Vendor\phpseclib3\Crypt\EC\BaseCurves\TwistedEdwards || $curve instanceof \WPStaging\Vendor\phpseclib3\Crypt\EC\BaseCurves\Montgomery) {
            throw new \WPStaging\Vendor\phpseclib3\Exception\UnsupportedCurveException('TwistedEdwards Curves are not supported');
        }
        $publicKey = "\4" . $publicKey[0]->toBytes() . $publicKey[1]->toBytes();
        $key = ['version' => 'ecPrivkeyVer1', 'privateKey' => $privateKey->toBytes(), 'parameters' => new \WPStaging\Vendor\phpseclib3\File\ASN1\Element(self::encodeParameters($curve)), 'publicKey' => "\0" . $publicKey];
