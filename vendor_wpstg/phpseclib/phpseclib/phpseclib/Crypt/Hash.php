...
class Hash
{
    ...
    public function setHash($hash)
    {
        ...
        if (\in_array(\substr($hash, 0, 5), ['sha3-', 'shake', 'kecca'])) {
            ...
            if (\version_compare(\PHP_VERSION, '7.1.0') < 0 || \substr($hash, 0, 5) != 'sha3-') {
                ...
                $hash = ['WPStaging\\Vendor\\phpseclib3\\Crypt\\Hash', \PHP_INT_SIZE == 8 ? 'sha3_64' : 'sha3_32'];
            }
        }
        if ($hash == 'sha512/224' || $hash == 'sha512/256') {
            ...
            if (\version_compare(\PHP_VERSION, '7.1.0') < 0) {
                ...
                $hash = ['WPStaging\\Vendor\\phpseclib3\\Crypt\\Hash', 'sha512'];
            }
        }
        if (\is_array($hash)) {
            $b = $this->blockSize >> 3;
    ...
    private static function L1Hash($k, $m)
    {
        ...
        //
        $length = new \WPStaging\Vendor\phpseclib3\Math\BigInteger(1024 * 8);
        $y = '';
        ...
        $m[$i] = \pack('N*', ...\unpack('V*', $m[$i]));
        // ENDIAN-SWAP
        $y .= static::nh($k, $m[$i], new \WPStaging\Vendor\phpseclib3\Math\BigInteger($length * 8));
        return $y;
    }
    /**
     * NH Algorithm
    ...
    {
        $toUInt32 = function ($x) {
            $x = new \WPStaging\Vendor\phpseclib3\Math\BigInteger($x, 256);
            $x->setPrecision(32);
            return $x;
        };
        //
        ...
        //
        $y = new \WPStaging\Vendor\phpseclib3\Math\BigInteger();
        $y->setPrecision(64);
        $i = 0;
        while ($i < $t) {
            $temp = $m[$i]->add($k[$i]);
