...
class Hash
{
    ...
    function _sha512($m)
    {
        ...
        if (!isset($k)) {
            ...
            for ($i = 0; $i < 8; $i++) {
                $init384[$i] = new \WPStaging\Vendor\phpseclib\Math\BigInteger($init384[$i], 16);
                $init384[$i]->setPrecision(64);
                $init512[$i] = new \WPStaging\Vendor\phpseclib\Math\BigInteger($init512[$i], 16);
            ...
            for ($i = 0; $i < 80; $i++) {
                $k[$i] = new \WPStaging\Vendor\phpseclib\Math\BigInteger($k[$i], 16);
            }
        }
        ...
        foreach ($chunks as $chunk) {
            $w = array();
            for ($i = 0; $i < 16; $i++) {
                $temp = new \WPStaging\Vendor\phpseclib\Math\BigInteger($this->_string_shift($chunk, 8), 256);
