...
use WPStaging\Vendor\phpseclib3\Exception\InconsistentSetupException;
    ...
    protected function handleOpenSSL($func, $message, $signature = null)
    {
        ...
        if (self::$forcedEngine !== 'PHP') {
            ...
            if ($this->{$paddingType} === self::SIGNATURE_PSS) {
                switch (\true) {
                    case !\defined('WPStaging\\Vendor\\OPENSSL_PKCS1_PSS_PADDING'):
            ...
            if (isset($error)) {
                ...
                switch (\true) {
                    case $this->signaturePadding === self::SIGNATURE_PSS && \defined('WPStaging\\Vendor\\OPENSSL_PKCS1_PSS_PADDING'):
