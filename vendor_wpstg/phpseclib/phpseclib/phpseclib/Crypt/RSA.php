 ...
 */
namespace WPStaging\Vendor\phpseclib\Crypt;
...
class RSA
{
    ...
    function __construct()
    {
        ...
        if (!\defined('WPStaging\\Vendor\\CRYPT_RSA_MODE')) {
            switch (\true) {
                ...
                case \defined('WPStaging\\Vendor\\MATH_BIGINTEGER_OPENSSL_DISABLE'):
                    \define('WPStaging\\Vendor\\CRYPT_RSA_MODE', self::MODE_INTERNAL);
                    break;
                case \function_exists('phpinfo') && \extension_loaded('openssl') && \file_exists($this->configFile):
                    ...
                    switch (\true) {
                        ...
                        case \version_compare($versions['Header'], '1.0.0') >= 0 && \version_compare($versions['Library'], '1.0.0') >= 0:
                            \define('WPStaging\\Vendor\\CRYPT_RSA_MODE', self::MODE_OPENSSL);
                            break;
                        default:
                            \define('WPStaging\\Vendor\\CRYPT_RSA_MODE', self::MODE_INTERNAL);
        ...
        $this->zero = new \WPStaging\Vendor\phpseclib\Math\BigInteger();
        $this->one = new \WPStaging\Vendor\phpseclib\Math\BigInteger(1);
        $this->hash = new \WPStaging\Vendor\phpseclib\Crypt\Hash('sha1');
        $this->hLen = $this->hash->getLength();
        $this->hashName = 'sha1';
        $this->mgfHash = new \WPStaging\Vendor\phpseclib\Crypt\Hash('sha1');
    ...
    {
        if (!\defined('WPStaging\\Vendor\\CRYPT_RSA_EXPONENT')) {
            // http://en.wikipedia.org/wiki/65537_%28number%29
            \define('WPStaging\\Vendor\\CRYPT_RSA_EXPONENT', '65537');
        }
        ...
        if (!\defined('WPStaging\\Vendor\\CRYPT_RSA_SMALLEST_PRIME')) {
            \define('WPStaging\\Vendor\\CRYPT_RSA_SMALLEST_PRIME', 4096);
        }
        ...
        static $e;
        if (!isset($e)) {
            $e = new \WPStaging\Vendor\phpseclib\Math\BigInteger(CRYPT_RSA_EXPONENT);
        }
        ...
        $generator = new \WPStaging\Vendor\phpseclib\Math\BigInteger();
        $n = $this->one->copy();
        if (!empty($partial)) {
        ...
        switch ($this->privateKeyFormat) {
            ...
            case self::PRIVATE_FORMAT_PUTTY:
                ...
                if (empty($this->password) && !\is_string($this->password)) {
                    ...
                    $private .= \WPStaging\Vendor\phpseclib\Crypt\Random::string(16 - (\strlen($private) & 15));
                    $source .= \pack('Na*', \strlen($private), $private);
                    $sequence = 0;
                    $symkey = '';
                    ...
                    $crypto = new \WPStaging\Vendor\phpseclib\Crypt\AES();
            ...
            case self::PRIVATE_FORMAT_OPENSSH:
                ...
                $checkint = \WPStaging\Vendor\phpseclib\Crypt\Random::string(4);
            ...
            default:
                ...
                if ($this->privateKeyFormat == self::PRIVATE_FORMAT_PKCS8) {
                    ...
                    $RSAPrivateKey = \pack('Ca*a*', self::ASN1_SEQUENCE, $this->_encodeLength(\strlen($RSAPrivateKey)), $RSAPrivateKey);
                    if (!empty($this->password) || \is_string($this->password)) {
                        $salt = \WPStaging\Vendor\phpseclib\Crypt\Random::string(8);
                        $iterationCount = 2048;
                        $crypto = new \WPStaging\Vendor\phpseclib\Crypt\DES();
                ...
                if (!empty($this->password) || \is_string($this->password)) {
                    $iv = \WPStaging\Vendor\phpseclib\Crypt\Random::string(8);
                    $symkey = \pack('H*', \md5($this->password . $iv));
                    // symkey is short for symmetric key
                    $symkey .= \substr(\pack('H*', \md5($symkey . $this->password . $iv)), 0, 8);
                    $des = new \WPStaging\Vendor\phpseclib\Crypt\TripleDES();
                    $des->setKey($symkey);
                    $des->setIV($iv);
        ...
        switch ($type) {
            ...
            case self::PUBLIC_FORMAT_PKCS1:
                ...
                if (\preg_match('#DEK-Info: (.+),(.+)#', $key, $matches)) {
                    ...
                    switch ($matches[1]) {
                        ...
                        case 'AES-128-CBC':
                            ...
                            break;
                        case 'DES-EDE3-CFB':
                            $crypto = new \WPStaging\Vendor\phpseclib\Crypt\TripleDES(\WPStaging\Vendor\phpseclib\Crypt\Base::MODE_CFB);
                            break;
                        case 'DES-EDE3-CBC':
                            $symkey = \substr($symkey, 0, 24);
                            $crypto = new \WPStaging\Vendor\phpseclib\Crypt\TripleDES();
                            break;
                        case 'DES-CBC':
                            $crypto = new \WPStaging\Vendor\phpseclib\Crypt\DES();
                            break;
                ...
                if (\strlen($temp) != 1 || \ord($temp) > 2) {
                    $components['modulus'] = new \WPStaging\Vendor\phpseclib\Math\BigInteger($temp, 256);
                    $this->_string_shift($key);
                    // skip over self::ASN1_INTEGER
                    $length = $this->_decodeLength($key);
                    $components[$type == self::PUBLIC_FORMAT_PKCS1 ? 'publicExponent' : 'privateExponent'] = new \WPStaging\Vendor\phpseclib\Math\BigInteger($this->_string_shift($key, $length), 256);
                    return $components;
                }
                ...
                $length = $this->_decodeLength($key);
                $components['modulus'] = new \WPStaging\Vendor\phpseclib\Math\BigInteger($this->_string_shift($key, $length), 256);
                $this->_string_shift($key);
                $length = $this->_decodeLength($key);
                $components['publicExponent'] = new \WPStaging\Vendor\phpseclib\Math\BigInteger($this->_string_shift($key, $length), 256);
                $this->_string_shift($key);
                $length = $this->_decodeLength($key);
                $components['privateExponent'] = new \WPStaging\Vendor\phpseclib\Math\BigInteger($this->_string_shift($key, $length), 256);
                $this->_string_shift($key);
                $length = $this->_decodeLength($key);
                $components['primes'] = array(1 => new \WPStaging\Vendor\phpseclib\Math\BigInteger($this->_string_shift($key, $length), 256));
                $this->_string_shift($key);
                $length = $this->_decodeLength($key);
                $components['primes'][] = new \WPStaging\Vendor\phpseclib\Math\BigInteger($this->_string_shift($key, $length), 256);
                $this->_string_shift($key);
                $length = $this->_decodeLength($key);
                $components['exponents'] = array(1 => new \WPStaging\Vendor\phpseclib\Math\BigInteger($this->_string_shift($key, $length), 256));
                $this->_string_shift($key);
                $length = $this->_decodeLength($key);
                $components['exponents'][] = new \WPStaging\Vendor\phpseclib\Math\BigInteger($this->_string_shift($key, $length), 256);
                $this->_string_shift($key);
                $length = $this->_decodeLength($key);
                $components['coefficients'] = array(2 => new \WPStaging\Vendor\phpseclib\Math\BigInteger($this->_string_shift($key, $length), 256));
                if (!empty($key)) {
                    if (\ord($this->_string_shift($key)) != self::ASN1_SEQUENCE) {
                        return \false;
                    }
                    ...
                    while (!empty($key)) {
                        ...
                        $this->_string_shift($key);
                        $length = $this->_decodeLength($key);
                        $components['coefficients'][] = new \WPStaging\Vendor\phpseclib\Math\BigInteger($this->_string_shift($key, $length), 256);
                    }
                }
                return $components;
            case self::PUBLIC_FORMAT_OPENSSH:
                ...
                $publicExponent = new \WPStaging\Vendor\phpseclib\Math\BigInteger($this->_string_shift($key, $length), -256);
                if (\strlen($key) <= 4) {
                    return \false;
                }
                \extract(\unpack('Nlength', $this->_string_shift($key, 4)));
                $modulus = new \WPStaging\Vendor\phpseclib\Math\BigInteger($this->_string_shift($key, $length), -256);
                if ($cleanup && \strlen($key)) {
                    if (\strlen($key) <= 4) {
                        return \false;
                    }
                    \extract(\unpack('Nlength', $this->_string_shift($key, 4)));
                    $realModulus = new \WPStaging\Vendor\phpseclib\Math\BigInteger($this->_string_shift($key, $length), -256);
            ...
            case self::PRIVATE_FORMAT_PUTTY:
                ...
                $components['publicExponent'] = new \WPStaging\Vendor\phpseclib\Math\BigInteger($this->_string_shift($public, $length), -256);
                \extract(\unpack('Nlength', $this->_string_shift($public, 4)));
                $components['modulus'] = new \WPStaging\Vendor\phpseclib\Math\BigInteger($this->_string_shift($public, $length), -256);
                ...
                $components['privateExponent'] = new \WPStaging\Vendor\phpseclib\Math\BigInteger($this->_string_shift($private, $length), -256);
                \extract(\unpack('Nlength', $this->_string_shift($private, 4)));
                if (\strlen($private) < $length) {
                    return \false;
                }
                $components['primes'] = array(1 => new \WPStaging\Vendor\phpseclib\Math\BigInteger($this->_string_shift($private, $length), -256));
                \extract(\unpack('Nlength', $this->_string_shift($private, 4)));
                if (\strlen($private) < $length) {
                    return \false;
                }
                $components['primes'][] = new \WPStaging\Vendor\phpseclib\Math\BigInteger($this->_string_shift($private, $length), -256);
                ...
                $components['coefficients'] = array(2 => new \WPStaging\Vendor\phpseclib\Math\BigInteger($this->_string_shift($private, $length), -256));
                return $components;
            case self::PRIVATE_FORMAT_OPENSSH:
                $components = array();
                $decoded = $this->_extractBER($key);
                ...
                switch ($ciphername) {
                    ...
                    case 'aes256-ctr':
                        ...
                        $crypto = new \WPStaging\Vendor\phpseclib\Crypt\AES(\WPStaging\Vendor\phpseclib\Crypt\AES::MODE_CTR);
                        $crypto->disablePadding();
                ...
                foreach ($values as &$value) {
                    ...
                    $value = new \WPStaging\Vendor\phpseclib\Math\BigInteger($this->_string_shift($paddedKey, $length), -256);
                }
                \extract(\unpack('Nlength', $this->_string_shift($paddedKey, 4)));
                if (\strlen($paddedKey) < $length) {
                    return \false;
    ...
    function _stop_element_handler($parser, $name)
    {
        if (isset($this->current)) {
            $this->current = new \WPStaging\Vendor\phpseclib\Math\BigInteger(\base64_decode($this->current), 256);
            unset($this->current);
        }
    }
    /**
    ...
    function loadKey($key, $type = \false)
    {
        if ($key instanceof \WPStaging\Vendor\phpseclib\Crypt\RSA) {
            ...
            if (\is_object($key->hash)) {
                $this->hash = new \WPStaging\Vendor\phpseclib\Crypt\Hash($key->hash->getHash());
            }
            if (\is_object($key->mgfHash)) {
                $this->mgfHash = new \WPStaging\Vendor\phpseclib\Crypt\Hash($key->mgfHash->getHash());
            }
    ...
    function getPublicKeyFingerprint($algorithm = 'md5')
    {
        ...
        switch ($algorithm) {
            case 'sha256':
                $hash = new \WPStaging\Vendor\phpseclib\Crypt\Hash('sha256');
    ...
    function _generateMinMax($bits)
    {
        ...
        return array('min' => new \WPStaging\Vendor\phpseclib\Math\BigInteger($min, 256), 'max' => new \WPStaging\Vendor\phpseclib\Math\BigInteger($max, 256));
    }
    /**
    ...
    function setHash($hash)
    {
        ...
        switch ($hash) {
            ...
            case 'sha384':
            case 'sha512':
                $this->hash = new \WPStaging\Vendor\phpseclib\Crypt\Hash($hash);
                $this->hashName = $hash;
                break;
            default:
                $this->hash = new \WPStaging\Vendor\phpseclib\Crypt\Hash('sha1');
    ...
    function setMGFHash($hash)
    {
        ...
        switch ($hash) {
            ...
            case 'sha384':
            case 'sha512':
                $this->mgfHash = new \WPStaging\Vendor\phpseclib\Crypt\Hash($hash);
                break;
            default:
                $this->mgfHash = new \WPStaging\Vendor\phpseclib\Crypt\Hash('sha1');
        }
    ...
    function _os2ip($x)
    {
        return new \WPStaging\Vendor\phpseclib\Math\BigInteger($x, 256);
    }
    /**
    ...
    function _exponentiate($x)
    {
        ...
        if (\defined('WPStaging\\Vendor\\CRYPT_RSA_DISABLE_BLINDING')) {
            ...
            $one = new \WPStaging\Vendor\phpseclib\Math\BigInteger(1);
    ...
    function _rsaes_oaep_encrypt($m, $l = '')
    {
        ...
        $db = $lHash . $ps . \chr(1) . $m;
        $seed = \WPStaging\Vendor\phpseclib\Crypt\Random::string($this->hLen);
    ...
    function _rsaes_pkcs1_v1_5_encrypt($m)
    {
        ...
        while (\strlen($ps) != $psLen) {
            $temp = \WPStaging\Vendor\phpseclib\Crypt\Random::string($psLen - \strlen($ps));
            $temp = \str_replace("\0", '', $temp);
            $ps .= $temp;
        }
        $type = 2;
        // see the comments of _rsaes_pkcs1_v1_5_decrypt() to understand why this is being done
        if (\defined('WPStaging\\Vendor\\CRYPT_RSA_PKCS15_COMPAT') && (!isset($this->publicExponent) || $this->exponent !== $this->publicExponent)) {
            $type = 1;
    ...
    function _emsa_pss_encode($m, $emBits)
    {
        ...
        $salt = \WPStaging\Vendor\phpseclib\Crypt\Random::string($sLen);
