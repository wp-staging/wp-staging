...
abstract class OpenSSH extends \WPStaging\Vendor\phpseclib3\Crypt\Common\Formats\Keys\OpenSSH
{
    ...
    public static function load($key, $password = '')
    {
        ...
        if (isset($parsed['paddedKey'])) {
            list($type) = \WPStaging\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('s', $parsed['paddedKey']);
            if ($type != $parsed['type']) {
                throw new \RuntimeException("The public and private keys are not of the same type ({$type} vs {$parsed['type']})");
            }
            $primes = $coefficients = [];
            list($modulus, $publicExponent, $privateExponent, $coefficients[2], $primes[1], $primes[2], $comment, ) = \WPStaging\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('i6s', $parsed['paddedKey']);
        ...
        list($publicExponent, $modulus) = \WPStaging\Vendor\phpseclib3\Common\Functions\Strings::unpackSSH2('ii', $parsed['publicKey']);
    ...
    {
        $RSAPublicKey = \WPStaging\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('sii', 'ssh-rsa', $e, $n);
    ...
    public static function savePrivateKey(\WPStaging\Vendor\phpseclib3\Math\BigInteger $n, \WPStaging\Vendor\phpseclib3\Math\BigInteger $e, \WPStaging\Vendor\phpseclib3\Math\BigInteger $d, array $primes, array $exponents, array $coefficients, $password = '', array $options = [])
    {
        ...
        $privateKey = \WPStaging\Vendor\phpseclib3\Common\Functions\Strings::packSSH2('si6', 'ssh-rsa', $n, $e, $d, $coefficients[2], $primes[1], $primes[2]);
