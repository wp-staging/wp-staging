 ...
 * @access  public
 */
class SFTP extends \WPStaging\Vendor\phpseclib\Net\SSH2
{
    /**
    ...
    function _precheck()
    {
        if (!($this->bitmap & \WPStaging\Vendor\phpseclib\Net\SSH2::MASK_LOGIN)) {
            return \false;
        }
        if ($this->pwd === \false) {
    ...
    function put($remote_file, $data, $mode = self::SOURCE_STRING, $start = -1, $local_start = -1, $progressCallback = null)
    {
        ...
        if ($mode & \WPStaging\Vendor\phpseclib\Net\SFTP::SOURCE_LOCAL_FILE) {
