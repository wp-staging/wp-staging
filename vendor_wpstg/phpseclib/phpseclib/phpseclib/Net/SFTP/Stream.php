 ...
 */
namespace WPStaging\Vendor\phpseclib\Net\SFTP;
...
class Stream
{
    ...
    {
        if (\defined('WPStaging\\Vendor\\NET_SFTP_STREAM_LOGGING')) {
            echo "__construct()\r\n";
        }
    }
    /**
    ...
    function _parse_path($path)
    {
        ...
        if ($host[0] == '$') {
            $host = \substr($host, 1);
            global ${$host};
            if (${$host} instanceof \WPStaging\Vendor\phpseclib\Net\SFTP === \false) {
                return \false;
            }
            $this->sftp = ${$host};
        } else {
            ...
            if (isset($sftp) && $sftp instanceof \WPStaging\Vendor\phpseclib\Net\SFTP) {
                $this->sftp = $sftp;
                return $path;
            }
            ...
            if (isset($context[$scheme]['privkey']) && $context[$scheme]['privkey'] instanceof \WPStaging\Vendor\phpseclib\Crypt\RSA) {
            ...
            if (isset(self::$instances[$host][$port][$user][(string) $pass])) {
                ...
                $this->sftp = new \WPStaging\Vendor\phpseclib\Net\SFTP($host, $port);
    ...
    function _stream_write($data)
    {
        ...
        $result = $this->sftp->put($this->path, $data, \WPStaging\Vendor\phpseclib\Net\SFTP::SOURCE_STRING, $this->pos);
        if (isset($this->notification) && \is_callable($this->notification)) {
            if (!$result) {
                \call_user_func($this->notification, \STREAM_NOTIFY_FAILURE, \STREAM_NOTIFY_SEVERITY_ERR, $this->sftp->getLastSFTPError(), NET_SFTP_OPEN, 0, 0);
                return 0;
