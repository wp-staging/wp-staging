...
use WPStaging\Vendor\phpseclib\Crypt\DES;
...
class SSH1
{
    ...
    function _connect()
    {
        ...
        if (\defined('WPStaging\\Vendor\\NET_SSH1_LOGGING')) {
        ...
        $server_key_public_exponent = new \WPStaging\Vendor\phpseclib\Math\BigInteger($this->_string_shift($response[self::RESPONSE_DATA], \ceil($temp['len'] / 8)), 256);
        $this->server_key_public_exponent = $server_key_public_exponent;
        if (\strlen($response[self::RESPONSE_DATA]) < 2) {
            return \false;
        }
        $temp = \unpack('nlen', $this->_string_shift($response[self::RESPONSE_DATA], 2));
        $server_key_public_modulus = new \WPStaging\Vendor\phpseclib\Math\BigInteger($this->_string_shift($response[self::RESPONSE_DATA], \ceil($temp['len'] / 8)), 256);
        ...
        $temp = \unpack('nlen', $this->_string_shift($response[self::RESPONSE_DATA], 2));
        $host_key_public_exponent = new \WPStaging\Vendor\phpseclib\Math\BigInteger($this->_string_shift($response[self::RESPONSE_DATA], \ceil($temp['len'] / 8)), 256);
        $this->host_key_public_exponent = $host_key_public_exponent;
        if (\strlen($response[self::RESPONSE_DATA]) < 2) {
            return \false;
        }
        $temp = \unpack('nlen', $this->_string_shift($response[self::RESPONSE_DATA], 2));
        $host_key_public_modulus = new \WPStaging\Vendor\phpseclib\Math\BigInteger($this->_string_shift($response[self::RESPONSE_DATA], \ceil($temp['len'] / 8)), 256);
        ...
        $session_key = \WPStaging\Vendor\phpseclib\Crypt\Random::string(32);
        ...
        switch ($cipher) {
            ...
            case self::CIPHER_DES:
                $this->crypto = new \WPStaging\Vendor\phpseclib\Crypt\DES();
                $this->crypto->disablePadding();
                $this->crypto->enableContinuousBuffer();
                $this->crypto->setKey(\substr($session_key, 0, 8));
                break;
            case self::CIPHER_3DES:
                $this->crypto = new \WPStaging\Vendor\phpseclib\Crypt\TripleDES(\WPStaging\Vendor\phpseclib\Crypt\TripleDES::MODE_3CBC);
                $this->crypto->disablePadding();
                $this->crypto->enableContinuousBuffer();
    ...
    function login($username, $password = '')
    {
        ...
        if (\defined('WPStaging\\Vendor\\NET_SSH1_LOGGING') && NET_SSH1_LOGGING == self::LOG_COMPLEX) {
            $data = \pack('CNa*', NET_SSH1_CMSG_AUTH_PASSWORD, \strlen('password'), 'password');
    ...
    function _send_binary_packet($data)
    {
        ...
        $padding = \WPStaging\Vendor\phpseclib\Crypt\Random::string(8 - ($length & 7));
        $orig = $data;
        $data = $padding . $data;
    ...
    function _rsa_crypt($m, $key)
    {
        ...
        */
        ...
        while (\strlen($random) != $length) {
            $block = \WPStaging\Vendor\phpseclib\Crypt\Random::string($length - \strlen($random));
            $block = \str_replace("\0", '', $block);
            $random .= $block;
        }
        $temp = \chr(0) . \chr(2) . $random . \chr(0) . $m;
        $m = new \WPStaging\Vendor\phpseclib\Math\BigInteger($temp, 256);
