...
class ASN1
{
    ...
    {
        if ($encoded instanceof \WPStaging\Vendor\phpseclib\File\ASN1\Element) {
    ...
    function _decode_ber($encoded, $start = 0, $encoded_pos = 0)
    {
        ...
        switch ($tag) {
            ...
            case self::TYPE_ENUMERATED:
                ...
                $current['content'] = new \WPStaging\Vendor\phpseclib\Math\BigInteger(\substr($content, $content_pos), -256);
                break;
            case self::TYPE_REAL:
    ...
    function asn1map($decoded, $mapping, $special = array())
    {
        ...
        switch (\true) {
            case $mapping['type'] == self::TYPE_ANY:
                ...
                if (isset($decoded['constant']) || !isset($this->ANYmap[$intype]) || \ord($this->encoded[$decoded['start']]) & 0x20) {
                    return new \WPStaging\Vendor\phpseclib\File\ASN1\Element(\substr($this->encoded, $decoded['start'], $decoded['length']));
                }
        ...
        switch ($decoded['type']) {
            ...
            case self::TYPE_ENUMERATED:
                ...
                if (isset($mapping['implicit'])) {
                    $temp = new \WPStaging\Vendor\phpseclib\Math\BigInteger($decoded['content'], -256);
                }
                if (isset($mapping['mapping'])) {
    ...
    {
        if ($source instanceof \WPStaging\Vendor\phpseclib\File\ASN1\Element) {
        ...
        switch ($tag) {
            ...
            case self::TYPE_ENUMERATED:
                if (!isset($mapping['mapping'])) {
                    if (\is_numeric($source)) {
                        $source = new \WPStaging\Vendor\phpseclib\Math\BigInteger($source);
                    }
                    ...
                    $value = new \WPStaging\Vendor\phpseclib\Math\BigInteger($value);
            ...
            case self::TYPE_ANY:
                ...
                switch (\true) {
                    ...
                    case \is_int($source):
                    case $source instanceof \WPStaging\Vendor\phpseclib\Math\BigInteger:
    ...
    function _decodeOID($content)
    {
        static $eighty;
        if (!$eighty) {
            $eighty = new \WPStaging\Vendor\phpseclib\Math\BigInteger(80);
        }
        $oid = array();
        $pos = 0;
        $len = \strlen($content);
        ...
        $n = new \WPStaging\Vendor\phpseclib\Math\BigInteger();
        while ($pos < $len) {
            $temp = \ord($content[$pos++]);
            $n = $n->bitwise_leftShift(7);
            $n = $n->bitwise_or(new \WPStaging\Vendor\phpseclib\Math\BigInteger($temp & 0x7f));
            if (~$temp & 0x80) {
                $oid[] = $n;
                $n = new \WPStaging\Vendor\phpseclib\Math\BigInteger();
            }
    ...
    function _encodeOID($source)
    {
        ...
        if (!$mask) {
            $mask = new \WPStaging\Vendor\phpseclib\Math\BigInteger(0x7f);
            $zero = new \WPStaging\Vendor\phpseclib\Math\BigInteger();
            $forty = new \WPStaging\Vendor\phpseclib\Math\BigInteger(40);
        }
        ...
        $first = new \WPStaging\Vendor\phpseclib\Math\BigInteger($part1);
        $first = $first->multiply($forty);
        $first = $first->add(new \WPStaging\Vendor\phpseclib\Math\BigInteger($part2));
        ...
        foreach ($parts as $part) {
            if (!$part) {
                ...
                $temp = '';
                $part = new \WPStaging\Vendor\phpseclib\Math\BigInteger($part);
