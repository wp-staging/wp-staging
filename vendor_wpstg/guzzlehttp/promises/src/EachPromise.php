 ...
 */
class EachPromise implements \WPStaging\Vendor\GuzzleHttp\Promise\PromisorInterface
{
    private $pending = [];
    ...
    {
        $this->iterable = \WPStaging\Vendor\GuzzleHttp\Promise\Create::iterFor($iterable);
    ...
    {
        $this->mutex = \false;
        $this->aggregate = new \WPStaging\Vendor\GuzzleHttp\Promise\Promise(function () {
            ...
            while ($promise = \current($this->pending)) {
                \next($this->pending);
                $promise->wait();
                if (\WPStaging\Vendor\GuzzleHttp\Promise\Is::settled($this->aggregate)) {
                    return;
                }
            }
        });
    ...
    private function addPending()
    {
        ...
        $promise = \WPStaging\Vendor\GuzzleHttp\Promise\Create::promiseFor($this->iterable->current());
