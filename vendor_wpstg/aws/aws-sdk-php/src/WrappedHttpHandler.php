...
class WrappedHttpHandler
{
     ...
     *                                 information.
     */
    public function __construct(callable $httpHandler, callable $parser, callable $errorParser, $exceptionClass = 'WPStaging\\Vendor\\Aws\\Exception\\AwsException', $collectStats = \false)
    {
        $this->httpHandler = $httpHandler;
        $this->parser = $parser;
        $this->errorParser = $errorParser;
        $this->exceptionClass = strpos("WPStaging\\Vendor", $exceptionClass) !== false ? $exceptionClass :  "WPStaging\\Vendor\\" . $exceptionClass;
    ...
    public function __invoke(\WPStaging\Vendor\Aws\CommandInterface $command, \WPStaging\Vendor\Psr\Http\Message\RequestInterface $request)
    {
        ...
        return \WPStaging\Vendor\GuzzleHttp\Promise\Create::promiseFor($fn($request, $options))->then(function (\WPStaging\Vendor\Psr\Http\Message\ResponseInterface $res) use($command, $request, &$stats) {
            ...
            return new \WPStaging\Vendor\GuzzleHttp\Promise\RejectedPromise($err);
        });
    }
    /**
     ...
     */
    private function parseResponse(\WPStaging\Vendor\Aws\CommandInterface $command, \WPStaging\Vendor\Psr\Http\Message\RequestInterface $request, \WPStaging\Vendor\Psr\Http\Message\ResponseInterface $response, array $stats)
    {
        $parser = $this->parser;
        $status = $response->getStatusCode();
        $result = $status < 300 ? $parser($command, $response) : new \WPStaging\Vendor\Aws\Result();
     ...
     * @return \Exception
     */
    private function parseError(array $err, \WPStaging\Vendor\Psr\Http\Message\RequestInterface $request, \WPStaging\Vendor\Aws\CommandInterface $command, array $stats)
    {
