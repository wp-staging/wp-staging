...
use WPStaging\Vendor\AWS\CRT\Auth\Signable;
use WPStaging\Vendor\AWS\CRT\Auth\SignatureType;
use WPStaging\Vendor\AWS\CRT\Auth\Signing;
use WPStaging\Vendor\AWS\CRT\Auth\SigningAlgorithm;
use WPStaging\Vendor\AWS\CRT\Auth\SigningConfigAWS;
use WPStaging\Vendor\AWS\CRT\Auth\StaticCredentialsProvider;
use WPStaging\Vendor\AWS\CRT\HTTP\Request;
use WPStaging\Vendor\Aws\Exception\CommonRuntimeException;
use WPStaging\Vendor\Aws\Exception\CouldNotCreateChecksumException;
 ...
 */
class SignatureV4 implements \WPStaging\Vendor\Aws\Signature\SignatureInterface
{
    use SignatureTrait;
     ...
     * {@inheritdoc}
     */
    public function signRequest(\WPStaging\Vendor\Psr\Http\Message\RequestInterface $request, \WPStaging\Vendor\Aws\Credentials\CredentialsInterface $credentials, $signingService = null)
    {
     ...
     */
    public static function convertPostToGet(\WPStaging\Vendor\Psr\Http\Message\RequestInterface $request, $additionalQueryParams = "")
    {
        if ($request->getMethod() !== 'POST') {
            throw new \InvalidArgumentException('Expected a POST request but ' . 'received a ' . $request->getMethod() . ' request.');
        }
        $sr = $request->withMethod('GET')->withBody(\WPStaging\Vendor\GuzzleHttp\Psr7\Utils::streamFor(''))->withoutHeader('Content-Type')->withoutHeader('Content-Length');
    ...
    protected function getPayload(\WPStaging\Vendor\Psr\Http\Message\RequestInterface $request)
    {
        ...
        if (!$request->getBody()->isSeekable()) {
            throw new \WPStaging\Vendor\Aws\Exception\CouldNotCreateChecksumException('sha256');
        }
        try {
            return \WPStaging\Vendor\GuzzleHttp\Psr7\Utils::hash($request->getBody(), 'sha256');
        } catch (\Exception $e) {
            throw new \WPStaging\Vendor\Aws\Exception\CouldNotCreateChecksumException('sha256', $e);
        }
    }
    protected function getPresignedPayload(\WPStaging\Vendor\Psr\Http\Message\RequestInterface $request)
    {
    ...
    private function createPresignedRequest(\WPStaging\Vendor\Psr\Http\Message\RequestInterface $request, \WPStaging\Vendor\Aws\Credentials\CredentialsInterface $credentials)
    {
    ...
    private function parseRequest(\WPStaging\Vendor\Psr\Http\Message\RequestInterface $request)
    {
        ...
        $uri = $request->getUri();
        return ['method' => $request->getMethod(), 'path' => $uri->getPath(), 'query' => \WPStaging\Vendor\GuzzleHttp\Psr7\Query::parse($uri->getQuery()), 'uri' => $uri, 'headers' => $request->getHeaders(), 'body' => $request->getBody(), 'version' => $request->getProtocolVersion()];
    }
    private function buildRequest(array $req)
    {
        if ($req['query']) {
            $req['uri'] = $req['uri']->withQuery(\WPStaging\Vendor\GuzzleHttp\Psr7\Query::build($req['query']));
        }
        return new \WPStaging\Vendor\GuzzleHttp\Psr7\Request($req['method'], $req['uri'], $req['headers'], $req['body'], $req['version']);
    }
    /**
     ...
     * @return RequestInterface
     */
    protected function signWithV4a(\WPStaging\Vendor\Aws\Credentials\CredentialsInterface $credentials, \WPStaging\Vendor\Psr\Http\Message\RequestInterface $request, $signingService)
    {
        if (!\extension_loaded('awscrt')) {
            throw new \WPStaging\Vendor\Aws\Exception\CommonRuntimeException("AWS Common Runtime for PHP is required to use Signature V4A" . ".  Please install it using the instructions found at" . " https://github.com/aws/aws-sdk-php/blob/master/CRT_INSTRUCTIONS.md");
        }
        $credentials_provider = new \WPStaging\Vendor\AWS\CRT\Auth\StaticCredentialsProvider(['access_key_id' => $credentials->getAccessKeyId(), 'secret_access_key' => $credentials->getSecretKey(), 'session_token' => $credentials->getSecurityToken()]);
        $sha = $this->getPayload($request);
        $signingConfig = new \WPStaging\Vendor\AWS\CRT\Auth\SigningConfigAWS(['algorithm' => \WPStaging\Vendor\AWS\CRT\Auth\SigningAlgorithm::SIGv4_ASYMMETRIC, 'signature_type' => \WPStaging\Vendor\AWS\CRT\Auth\SignatureType::HTTP_REQUEST_HEADERS, 'credentials_provider' => $credentials_provider, 'signed_body_value' => $sha, 'region' => "*", 'service' => $signingService, 'date' => \time()]);
        $sha = $this->getPayload($request);
        ...
        $http_request = new \WPStaging\Vendor\AWS\CRT\HTTP\Request($request->getMethod(), (string) $request->getUri(), [], \array_map(function ($header) {
            return $header[0];
        }, $request->getHeaders()));
        \WPStaging\Vendor\AWS\CRT\Auth\Signing::signRequestAws(\WPStaging\Vendor\AWS\CRT\Auth\Signable::fromHttpRequest($http_request), $signingConfig, function ($signing_result, $error_code) use(&$http_request) {
