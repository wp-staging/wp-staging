...
use WPStaging\Vendor\Aws\Exception\UnresolvedSignatureException;
/**
 * Signature providers.
 *
...
class SignatureProvider
{
    ...
    public static function resolve(callable $provider, $version, $service, $region)
    {
        ...
        if ($result instanceof \WPStaging\Vendor\Aws\Signature\SignatureInterface) {
            return $result;
        }
        throw new \WPStaging\Vendor\Aws\Exception\UnresolvedSignatureException("Unable to resolve a signature for {$version}/{$service}/{$region}.\n" . "Valid signature versions include v4 and anonymous.");
    }
    /**
    ...
    public static function version()
    {
        return function ($version, $service, $region) {
            switch ($version) {
                case 's3v4':
                case 'v4':
                    return !empty(self::$s3v4SignedServices[$service]) ? new \WPStaging\Vendor\Aws\Signature\S3SignatureV4($service, $region) : new \WPStaging\Vendor\Aws\Signature\SignatureV4($service, $region);
                case 'v4a':
                    return new \WPStaging\Vendor\Aws\Signature\SignatureV4($service, $region, ['use_v4a' => \true]);
                case 'v4-unsigned-body':
                    return !empty(self::$s3v4SignedServices[$service]) ? new \WPStaging\Vendor\Aws\Signature\S3SignatureV4($service, $region, ['unsigned-body' => 'true']) : new \WPStaging\Vendor\Aws\Signature\SignatureV4($service, $region, ['unsigned-body' => 'true']);
                case 'anonymous':
                    return new \WPStaging\Vendor\Aws\Signature\AnonymousSignature();
                default:
                    return null;
            }
        };
