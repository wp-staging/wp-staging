...
class MockHandler implements \Countable
{
    ...
    public function append()
    {
        foreach (\func_get_args() as $value) {
            if ($value instanceof \WPStaging\Vendor\Aws\ResultInterface || $value instanceof \Exception || \is_callable($value)) {
                $this->queue[] = $value;
            } else {
    ...
    public function __invoke(\WPStaging\Vendor\Aws\CommandInterface $command, \WPStaging\Vendor\Psr\Http\Message\RequestInterface $request)
    {
        ...
        if ($result instanceof \Exception) {
            $result = new \WPStaging\Vendor\GuzzleHttp\Promise\RejectedPromise($result);
        } else {
            ...
            $result = \WPStaging\Vendor\GuzzleHttp\Promise\Create::promiseFor($result);
        }
