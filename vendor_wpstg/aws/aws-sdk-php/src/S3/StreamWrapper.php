...
use WPStaging\Vendor\GuzzleHttp\Psr7\CachingStream;
...
class StreamWrapper
{
     ...
     */
    public static function register(\WPStaging\Vendor\Aws\S3\S3ClientInterface $client, $protocol = 's3', \WPStaging\Vendor\Aws\CacheInterface $cache = null)
    {
        ...
        if ($cache) {
            ...
            $default[$protocol]['cache'] = new \WPStaging\Vendor\Aws\LruArrayCache();
        }
    ...
    public function stream_flush()
    {
        ...
        if (!isset($params['ContentType']) && ($type = \WPStaging\Vendor\GuzzleHttp\Psr7\MimeType::fromFilename($params['Key']))) {
    ...
    public function dir_opendir($path, $options)
    {
        ...
        $this->objectIterator = \WPStaging\Vendor\Aws\flatmap($this->getClient()->getPaginator('ListObjects', $op), function (\WPStaging\Vendor\Aws\Result $result) use($filterFn) {
    ...
    private function openReadStream()
    {
        ...
        if ($this->getOption('seekable') && !$this->body->isSeekable()) {
            $this->body = new \WPStaging\Vendor\GuzzleHttp\Psr7\CachingStream($this->body);
        }
        return \true;
    }
    private function openWriteStream()
    {
        $this->body = new \WPStaging\Vendor\GuzzleHttp\Psr7\Stream(\fopen('php://temp', 'r+'));
        return \true;
    }
    ...
    {
        if (!$this->cache) {
            $this->cache = $this->getOption('cache') ?: new \WPStaging\Vendor\Aws\LruArrayCache();
        }
        return $this->cache;
    }
    /**
