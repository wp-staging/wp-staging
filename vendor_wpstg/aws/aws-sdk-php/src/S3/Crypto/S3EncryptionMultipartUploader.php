...
use WPStaging\Vendor\Aws\S3\MultipartUploader;
use WPStaging\Vendor\Aws\S3\S3ClientInterface;
 ...
 * @deprecated
 */
class S3EncryptionMultipartUploader extends \WPStaging\Vendor\Aws\S3\MultipartUploader
{
    ...
    {
        return \in_array($cipherName, \WPStaging\Vendor\Aws\Crypto\AbstractCryptoClient::$supportedCiphers);
    }
    private $provider;
    ...
    private function getEncryptingDataPreparer()
    {
        return function () {
            ...
            list($this->source, $params) = \WPStaging\Vendor\GuzzleHttp\Promise\Create::promiseFor($this->encrypt($this->source, $this->config['@cipheroptions'] ?: [], $this->provider, $envelope))->then(function ($bodyStream) use($envelope) {
