 ...
 * </code>
 */
class S3EncryptionClientV2 extends \WPStaging\Vendor\Aws\Crypto\AbstractCryptoClientV2
{
     ...
     */
    public function __construct(\WPStaging\Vendor\Aws\S3\S3Client $client, $instructionFileSuffix = null)
    {
    ...
    public function putObjectAsync(array $args)
    {
        ...
        return \WPStaging\Vendor\GuzzleHttp\Promise\Create::promiseFor($this->encrypt(\WPStaging\Vendor\GuzzleHttp\Psr7\Utils::streamFor($args['Body']), $args, $provider, $envelope))->then(function ($encryptedBodyStream) use($args) {
            $hash = new \WPStaging\Vendor\Aws\PhpHash('sha256');
            $hashingEncryptedBodyStream = new \WPStaging\Vendor\Aws\HashingStream($encryptedBodyStream, $hash, self::getContentShaDecorator($args));
    ...
    public function getObjectAsync(array $args)
    {
        ...
        if (!isset($args['@SecurityProfile']) || !\in_array($args['@SecurityProfile'], self::$supportedSecurityProfiles)) {
            throw new \WPStaging\Vendor\Aws\Exception\CryptoException("@SecurityProfile is required and must be" . " set to 'V2' or 'V2_AND_LEGACY'");
        }
