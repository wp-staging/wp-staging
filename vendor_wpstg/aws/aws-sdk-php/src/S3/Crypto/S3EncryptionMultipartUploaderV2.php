 ...
 */
class S3EncryptionMultipartUploaderV2 extends \WPStaging\Vendor\Aws\S3\MultipartUploader
{
    ...
    {
        return \in_array($cipherName, \WPStaging\Vendor\Aws\Crypto\AbstractCryptoClientV2::$supportedCiphers);
    }
    private $provider;
    ...
    private function getEncryptingDataPreparer()
    {
        return function () {
            ...
            list($this->source, $params) = \WPStaging\Vendor\GuzzleHttp\Promise\Create::promiseFor($this->encrypt($this->source, $this->config ?: [], $this->provider, $envelope))->then(function ($bodyStream) use($envelope) {
