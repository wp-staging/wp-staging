...
use WPStaging\Vendor\Aws\Signature\SignatureTrait;
use WPStaging\Vendor\Aws\Signature\SignatureV4 as SignatureV4;
use WPStaging\Vendor\Aws\Api\TimestampShape as TimestampShape;
/**
...
class PostObjectV4
{
     ...
     */
    public function __construct(\WPStaging\Vendor\Aws\S3\S3ClientInterface $client, $bucket, array $formInputs, array $options = [], $expiration = '+1 hours')
    {
        $this->client = $client;
        $this->bucket = $bucket;
        ...
        // setup basic policy
        $policy = ['expiration' => \WPStaging\Vendor\Aws\Api\TimestampShape::format($expiration, 'iso8601'), 'conditions' => $options];
    ...
    {
        $uri = new \WPStaging\Vendor\GuzzleHttp\Psr7\Uri($this->client->getEndpoint());
    ...
    protected function getPolicyAndSignature(\WPStaging\Vendor\Aws\Credentials\CredentialsInterface $credentials, array $policy)
    {
        $ldt = \gmdate(\WPStaging\Vendor\Aws\Signature\SignatureV4::ISO8601_BASIC);
        $sdt = \substr($ldt, 0, 8);
