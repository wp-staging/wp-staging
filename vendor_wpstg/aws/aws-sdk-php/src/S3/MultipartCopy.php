...
use WPStaging\Vendor\Aws\Multipart\AbstractUploadManager;
use WPStaging\Vendor\Aws\ResultInterface;
use WPStaging\Vendor\GuzzleHttp\Psr7;
class MultipartCopy extends \WPStaging\Vendor\Aws\Multipart\AbstractUploadManager
{
    ...
    {
        if ($this->config['source_metadata'] instanceof \WPStaging\Vendor\Aws\ResultInterface) {
        ...
        if (\is_array($this->source)) {
            ...
            if (\strpos($key, '?')) {
                ...
                $headParams['Key'] = $key;
                $query = \WPStaging\Vendor\GuzzleHttp\Psr7\Query::parse($query, \false);
    ...
    {
        if (\WPStaging\Vendor\Aws\Arn\ArnParser::isArn($inputSource)) {
            $sourceBuilder = '';
        } else {
            $sourceBuilder = "/";
        }
