...
class ApplyChecksumMiddleware
{
     ...
     * @return callable
     */
    public static function wrap(\WPStaging\Vendor\Aws\Api\Service $api)
    {
        return function (callable $handler) use($api) {
            return new self($handler, $api);
        };
    }
    public function __construct(callable $nextHandler, \WPStaging\Vendor\Aws\Api\Service $api)
    {
        $this->api = $api;
    ...
    public function __invoke(\WPStaging\Vendor\Aws\CommandInterface $command, \WPStaging\Vendor\Psr\Http\Message\RequestInterface $request)
    {
        ...
        if (!empty($checksumInfo)) {
            ...
            if (!empty($checksumRequired) && !$request->hasHeader('Content-MD5')) {
                ...
                $request = $request->withHeader('Content-MD5', \base64_encode(\WPStaging\Vendor\GuzzleHttp\Psr7\Utils::hash($body, 'md5', \true)));
                return $next($command, $request);
            }
        }
