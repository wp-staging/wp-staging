...
use WPStaging\Vendor\Aws\HashingStream;
use WPStaging\Vendor\Aws\Multipart\AbstractUploader;
use WPStaging\Vendor\Aws\PhpHash;
use WPStaging\Vendor\Aws\ResultInterface;
use WPStaging\Vendor\GuzzleHttp\Psr7;
use WPStaging\Vendor\Psr\Http\Message\StreamInterface as Stream;
use WPStaging\Vendor\Aws\S3\Exception\S3MultipartUploadException;
/**
 * Encapsulates the execution of a multipart upload to S3 or Glacier.
 */
class MultipartUploader extends \WPStaging\Vendor\Aws\Multipart\AbstractUploader
{
     ...
     */
    public function __construct(\WPStaging\Vendor\Aws\S3\S3ClientInterface $client, $source, array $config = [])
    {
        parent::__construct($client, $source, \array_change_key_case($config) + ['bucket' => null, 'key' => null, 'exception_class' => \WPStaging\Vendor\Aws\S3\Exception\S3MultipartUploadException::class]);
    }
    ...
    protected function createPart($seekable, $number)
    {
        ...
        if ($seekable) {
            ...
            $body = $this->limitPartStream(new \WPStaging\Vendor\GuzzleHttp\Psr7\LazyOpenStream($this->source->getMetadata('uri'), 'r'));
        } else {
            ...
            $body = \WPStaging\Vendor\GuzzleHttp\Psr7\Utils::streamFor();
            \WPStaging\Vendor\GuzzleHttp\Psr7\Utils::copyToStream($source, $body);
        }
    ...
    protected function extractETag(\WPStaging\Vendor\Aws\ResultInterface $result)
    {
        return $result['ETag'];
    }
    protected function getSourceMimeType()
    {
        if ($uri = $this->source->getMetadata('uri')) {
            return \WPStaging\Vendor\GuzzleHttp\Psr7\MimeType::fromFilename($uri) ?: 'application/octet-stream';
        }
    }
     ...
     * @return Stream
     */
    private function decorateWithHashes(\WPStaging\Vendor\Psr\Http\Message\StreamInterface $stream, array &$data)
    {
        // Decorate source with a hashing stream
        $hash = new \WPStaging\Vendor\Aws\PhpHash('sha256');
        return new \WPStaging\Vendor\Aws\HashingStream($stream, $hash, function ($result) use(&$data) {
