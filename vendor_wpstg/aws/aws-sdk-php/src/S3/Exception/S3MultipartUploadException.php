...
class S3MultipartUploadException extends \WPStaging\Vendor\Aws\Exception\MultipartUploadException
{
     ...
     */
    public function __construct(\WPStaging\Vendor\Aws\Multipart\UploadState $state, $prev = null)
    {
        if (\is_array($prev) && ($error = $prev[\key($prev)])) {
            $this->collectPathInfo($error->getCommand());
        } elseif ($prev instanceof \WPStaging\Vendor\Aws\Exception\AwsException) {
     ...
     */
    private function collectPathInfo(\WPStaging\Vendor\Aws\CommandInterface $cmd)
    {
