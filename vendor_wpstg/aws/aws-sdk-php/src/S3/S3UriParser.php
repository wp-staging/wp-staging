...
class S3UriParser
{
    ...
    public function parse($uri)
    {
        ...
        if (!empty($components)) {
            if (\WPStaging\Vendor\Aws\Arn\ArnParser::isArn($components['host'])) {
                $arn = new \WPStaging\Vendor\Aws\Arn\S3\AccessPointArn($components['host']);
                return ['bucket' => $components['host'], 'key' => $components['path'], 'path_style' => \false, 'region' => $arn->getRegion()];
            }
        }
        $url = \WPStaging\Vendor\GuzzleHttp\Psr7\Utils::uriFor($uri);
    ...
    private function parseStreamWrapper(\WPStaging\Vendor\Psr\Http\Message\UriInterface $url)
    {
    ...
    private function parseCustomEndpoint(\WPStaging\Vendor\Psr\Http\Message\UriInterface $url)
    {
        $result = self::$defaultResult;
    ...
    private function parsePathStyle(\WPStaging\Vendor\Psr\Http\Message\UriInterface $url)
    {
        $result = self::$defaultResult;
        if ($url->getPath() != '/') {
            $path = \ltrim($url->getPath(), '/');
    ...
    private function parseVirtualHosted(\WPStaging\Vendor\Psr\Http\Message\UriInterface $url, array $matches)
    {
        $result = self::$defaultResult;
        $result['path_style'] = \false;
