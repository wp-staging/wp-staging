...
use WPStaging\Vendor\Aws\Command;
use WPStaging\Vendor\Aws\Exception\AwsException;
use WPStaging\Vendor\Aws\HandlerList;
use WPStaging\Vendor\Aws\InputValidationMiddleware;
use WPStaging\Vendor\Aws\Middleware;
use WPStaging\Vendor\Aws\Retry\QuotaManager;
use WPStaging\Vendor\Aws\RetryMiddleware;
use WPStaging\Vendor\Aws\ResultInterface;
use WPStaging\Vendor\Aws\CommandInterface;
use WPStaging\Vendor\Aws\RetryMiddlewareV2;
use WPStaging\Vendor\Aws\S3\UseArnRegion\Configuration;
use WPStaging\Vendor\Aws\S3\UseArnRegion\ConfigurationInterface;
use WPStaging\Vendor\Aws\S3\UseArnRegion\ConfigurationProvider as UseArnRegionConfigurationProvider;
use WPStaging\Vendor\Aws\S3\RegionalEndpoint\ConfigurationProvider;
use WPStaging\Vendor\GuzzleHttp\Exception\RequestException;
use WPStaging\Vendor\GuzzleHttp\Promise\Promise;
 ...
 */
class S3Client extends \WPStaging\Vendor\Aws\AwsClient implements \WPStaging\Vendor\Aws\S3\S3ClientInterface
{
    use S3ClientTrait;
    /** @var array */
    ...
    {
        if (!isset($args['s3_us_east_1_regional_endpoint']) || $args['s3_us_east_1_regional_endpoint'] instanceof \WPStaging\Vendor\Aws\CacheInterface) {
            $args['s3_us_east_1_regional_endpoint'] = \WPStaging\Vendor\Aws\S3\RegionalEndpoint\ConfigurationProvider::defaultProvider($args);
        }
        parent::__construct($args);
        $stack = $this->getHandlerList();
        $stack->appendInit(\WPStaging\Vendor\Aws\S3\SSECMiddleware::wrap($this->getEndpoint()->getScheme()), 's3.ssec');
        $stack->appendBuild(\WPStaging\Vendor\Aws\S3\ApplyChecksumMiddleware::wrap($this->getApi()), 's3.checksum');
        $stack->appendBuild(\WPStaging\Vendor\Aws\Middleware::contentType(['PutObject', 'UploadPart']), 's3.content_type');
        // Use the bucket style middleware when using a "bucket_endpoint" (for cnames)
        if ($this->getConfig('bucket_endpoint')) {
            $stack->appendBuild(\WPStaging\Vendor\Aws\S3\BucketEndpointMiddleware::wrap(), 's3.bucket_endpoint');
        } else {
            $stack->appendBuild(\WPStaging\Vendor\Aws\S3\S3EndpointMiddleware::wrap($this->getRegion(), $this->getConfig('endpoint_provider'), ['accelerate' => $this->getConfig('use_accelerate_endpoint'), 'path_style' => $this->getConfig('use_path_style_endpoint'), 'use_fips_endpoint' => $this->getConfig('use_fips_endpoint'), 'dual_stack' => $this->getConfig('use_dual_stack_endpoint')->isUseDualStackEndpoint()]), 's3.endpoint_middleware');
        }
        $stack->appendBuild(\WPStaging\Vendor\Aws\S3\BucketEndpointArnMiddleware::wrap($this->getApi(), $this->getRegion(), ['use_arn_region' => $this->getConfig('use_arn_region'), 'accelerate' => $this->getConfig('use_accelerate_endpoint'), 'path_style' => $this->getConfig('use_path_style_endpoint'), 'dual_stack' => $this->getConfig('use_dual_stack_endpoint')->isUseDualStackEndpoint(), 'use_fips_endpoint' => $this->getConfig('use_fips_endpoint'), 'disable_multiregion_access_points' => $this->getConfig('disable_multiregion_access_points'), 'endpoint' => isset($args['endpoint']) ? $args['endpoint'] : null]), 's3.bucket_endpoint_arn');
        $stack->appendValidate(\WPStaging\Vendor\Aws\InputValidationMiddleware::wrap($this->getApi(), self::$mandatoryAttributes), 'input_validation_middleware');
        $stack->appendSign(\WPStaging\Vendor\Aws\S3\PutObjectUrlMiddleware::wrap(), 's3.put_object_url');
        $stack->appendSign(\WPStaging\Vendor\Aws\S3\PermanentRedirectMiddleware::wrap(), 's3.permanent_redirect');
        $stack->appendInit(\WPStaging\Vendor\Aws\Middleware::sourceFile($this->getApi()), 's3.source_file');
    ...
    public static function _apply_use_arn_region($value, array &$args, \WPStaging\Vendor\Aws\HandlerList $list)
    {
        if ($value instanceof \WPStaging\Vendor\Aws\CacheInterface) {
            $value = \WPStaging\Vendor\Aws\S3\UseArnRegion\ConfigurationProvider::defaultProvider($args);
        }
        ...
        if ($value instanceof \WPStaging\Vendor\Aws\S3\UseArnRegion\ConfigurationInterface) {
            $args['use_arn_region'] = $value;
        } else {
            // The Configuration class itself will validate other inputs
            $args['use_arn_region'] = new \WPStaging\Vendor\Aws\S3\UseArnRegion\Configuration($value);
        }
    }
    public function createPresignedRequest(\WPStaging\Vendor\Aws\CommandInterface $command, $expires, array $options = [])
    {
        ...
        return $signer->presign(\WPStaging\Vendor\Aws\serialize($command), $this->getCredentials()->wait(), $expires, $options);
    }
    /**
    ...
    public function getObjectUrl($bucket, $key)
    {
        ...
        return (string) \WPStaging\Vendor\Aws\serialize($command)->getUri();
    }
    /**
    ...
    private function getLocationConstraintMiddleware()
    {
        ...
        return static function (callable $handler) use($region) {
            return function (\WPStaging\Vendor\Aws\Command $command, $request = null) use($handler, $region) {
    ...
    private function getSaveAsParameter()
    {
        return static function (callable $handler) {
            return function (\WPStaging\Vendor\Aws\Command $command, $request = null) use($handler) {
    ...
    private function getEncodingTypeMiddleware()
    {
        return static function (callable $handler) {
            return function (\WPStaging\Vendor\Aws\Command $command, $request = null) use($handler) {
                ...
                return $handler($command, $request)->then(function (\WPStaging\Vendor\Aws\ResultInterface $result) use($autoSet) {
    ...
    /** @internal */
    public static function _applyRetryConfig($value, $args, \WPStaging\Vendor\Aws\HandlerList $list)
    {
        if ($value) {
            $config = \WPStaging\Vendor\Aws\Retry\ConfigurationProvider::unwrap($value);
            if ($config->getMode() === 'legacy') {
                $maxRetries = $config->getMaxAttempts() - 1;
                $decider = \WPStaging\Vendor\Aws\RetryMiddleware::createDefaultDecider($maxRetries);
                $decider = function ($retries, $command, $request, $result, $error) use($decider, $maxRetries) {
                    ...
                    if ($error instanceof \WPStaging\Vendor\Aws\Exception\AwsException && $retries < $maxRetries) {
                        ...
                        if ($error->getPrevious() instanceof \WPStaging\Vendor\GuzzleHttp\Exception\RequestException) {
                ...
                $delay = [\WPStaging\Vendor\Aws\RetryMiddleware::class, 'exponentialDelay'];
                $list->appendSign(\WPStaging\Vendor\Aws\Middleware::retry($decider, $delay), 'retry');
            } else {
                $defaultDecider = \WPStaging\Vendor\Aws\RetryMiddlewareV2::createDefaultDecider(new \WPStaging\Vendor\Aws\Retry\QuotaManager(), $config->getMaxAttempts());
                $list->appendSign(\WPStaging\Vendor\Aws\RetryMiddlewareV2::wrap($config, ['collect_stats' => $args['stats']['retries'], 'decider' => function ($attempts, \WPStaging\Vendor\Aws\CommandInterface $cmd, $result) use($defaultDecider, $config) {
                    $isRetryable = $defaultDecider($attempts, $cmd, $result);
                    if (!$isRetryable && $result instanceof \WPStaging\Vendor\Aws\Exception\AwsException && $attempts < $config->getMaxAttempts()) {
                        if (!empty($result->getResponse()) && \strpos($result->getResponse()->getBody(), 'Your socket connection to the server') !== \false) {
                            $isRetryable = \false;
                        }
                        if ($result->getPrevious() instanceof \WPStaging\Vendor\GuzzleHttp\Exception\RequestException && $cmd->getName() !== 'CompleteMultipartUpload') {
                            $isRetryable = \true;
                        }
                    }
                    return $isRetryable;
    ...
    /** @internal */
    public static function _applyApiProvider($value, array &$args, \WPStaging\Vendor\Aws\HandlerList $list)
    {
        \WPStaging\Vendor\Aws\ClientResolver::_apply_api_provider($value, $args);
        $args['parser'] = new \WPStaging\Vendor\Aws\S3\GetBucketLocationParser(new \WPStaging\Vendor\Aws\S3\ValidateResponseChecksumParser(new \WPStaging\Vendor\Aws\S3\AmbiguousSuccessParser(new \WPStaging\Vendor\Aws\S3\RetryableMalformedResponseParser($args['parser'], $args['exception_class']), $args['error_parser'], $args['exception_class']), $args['api']));
    }
    /**
     * @internal
     * @codeCoverageIgnore
    ...
    public static function applyDocFilters(array $api, array $docs)
    {
        ...
        return [new \WPStaging\Vendor\Aws\Api\Service($api, \WPStaging\Vendor\Aws\Api\ApiProvider::defaultProvider()), new \WPStaging\Vendor\Aws\Api\DocModel($docs)];
    }
    /**
     * @internal
     * @codeCoverageIgnore
