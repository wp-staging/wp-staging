...
use WPStaging\Vendor\Aws\MultiRegionClient as BaseClient;
 ...
 */
class S3MultiRegionClient extends \WPStaging\Vendor\Aws\MultiRegionClient implements \WPStaging\Vendor\Aws\S3\S3ClientInterface
{
    use S3ClientTrait;
    /** @var CacheInterface */
    private $cache;
    public static function getArguments()
    {
        ...
        unset($args['region']);
        return $args + ['bucket_region_cache' => ['type' => 'config', 'valid' => [\WPStaging\Vendor\Aws\CacheInterface::class], 'doc' => 'Cache of regions in which given buckets are located.', 'default' => function () {
            return new \WPStaging\Vendor\Aws\LruArrayCache();
        }], 'region' => $regionDef];
    }
    ...
    private function determineRegionMiddleware()
    {
        return function (callable $handler) {
            return function (\WPStaging\Vendor\Aws\CommandInterface $command) use($handler) {
                $cacheKey = $this->getCacheKey($command['Bucket']);
                if (empty($command['@region']) && ($region = $this->cache->get($cacheKey))) {
                    $command['@region'] = $region;
                }
                return \WPStaging\Vendor\GuzzleHttp\Promise\Coroutine::of(function () use($handler, $command, $cacheKey) {
                    try {
                        (yield $handler($command));
                    } catch (\WPStaging\Vendor\Aws\S3\Exception\PermanentRedirectException $e) {
    ...
    public function determineBucketRegionAsync($bucketName)
    {
        ...
        if ($cached = $this->cache->get($cacheKey)) {
            return \WPStaging\Vendor\GuzzleHttp\Promise\Create::promiseFor($cached);
        }
