...
class RetryMiddleware
{
    ...
    public static function createDefaultDecider($maxRetries = 3, $extraConfig = [])
    {
        ...
        return function ($retries, \WPStaging\Vendor\Aws\CommandInterface $command, \WPStaging\Vendor\Psr\Http\Message\RequestInterface $request, \WPStaging\Vendor\Aws\ResultInterface $result = null, $error = null) use($maxRetries, $retryCurlErrors, $extraConfig) {
            ...
            if ($retries >= $maxRetries) {
                if (!empty($error) && $error instanceof \WPStaging\Vendor\Aws\Exception\AwsException && $isRetryable) {
    ...
    private static function isRetryable($result, $error, $retryCurlErrors, $extraConfig = [])
    {
        ...
        if (!$error instanceof \WPStaging\Vendor\Aws\Exception\AwsException) {
            return \false;
        }
        ...
        if (\count($retryCurlErrors) && ($previous = $error->getPrevious()) && $previous instanceof \WPStaging\Vendor\GuzzleHttp\Exception\RequestException) {
    ...
    public function __invoke(\WPStaging\Vendor\Aws\CommandInterface $command, \WPStaging\Vendor\Psr\Http\Message\RequestInterface $request = null)
    {
        ...
        $g = function ($value) use($handler, $decider, $delay, $command, $request, &$retries, &$requestStats, &$monitoringEvents, &$g) {
            ...
            } elseif ($value instanceof \WPStaging\Vendor\Aws\ResultInterface && !$decider($retries, $command, $request, $value, null)) {
