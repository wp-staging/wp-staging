...
use WPStaging\Vendor\Aws\Retry\ConfigurationInterface;
use WPStaging\Vendor\Aws\Retry\QuotaManager;
use WPStaging\Vendor\Aws\Retry\RateLimiter;
use WPStaging\Vendor\Aws\Retry\RetryHelperTrait;
...
class RetryMiddlewareV2
{
    ...
    public static function createDefaultDecider(\WPStaging\Vendor\Aws\Retry\QuotaManager $quotaManager, $maxAttempts = 3, $options = [])
    {
        $retryCurlErrors = [];
        if (\extension_loaded('curl')) {
            $retryCurlErrors[\CURLE_RECV_ERROR] = \true;
        }
        return function ($attempts, \WPStaging\Vendor\Aws\CommandInterface $command, $result) use($options, $quotaManager, $retryCurlErrors, $maxAttempts) {
            ...
            if ($isRetryable) {
                ...
                if ($attempts >= $maxAttempts) {
                    if (!empty($result) && $result instanceof \WPStaging\Vendor\Aws\Exception\AwsException) {
    ...
    public function __construct(\WPStaging\Vendor\Aws\Retry\ConfigurationInterface $config, callable $handler, $options = [])
    {
        $this->options = $options;
        $this->maxAttempts = $config->getMaxAttempts();
        $this->mode = $config->getMode();
        $this->nextHandler = $handler;
        $this->quotaManager = new \WPStaging\Vendor\Aws\Retry\QuotaManager();
        ...
        if ($this->mode === 'adaptive') {
            $this->rateLimiter = isset($options['rate_limiter']) ? $options['rate_limiter'] : new \WPStaging\Vendor\Aws\Retry\RateLimiter();
        }
    }
        ...
        $callback = function ($value) use($handler, $cmd, $req, $decider, $delayer, &$attempts, &$requestStats, &$monitoringEvents, &$callback) {
            ...
            if ($value instanceof \Exception || $value instanceof \Throwable) {
                if (!$decider($attempts, $cmd, $value)) {
                    return \WPStaging\Vendor\GuzzleHttp\Promise\Create::rejectionFor($this->bindStatsToReturn($value, $requestStats));
                }
            } elseif ($value instanceof \WPStaging\Vendor\Aws\ResultInterface && !$decider($attempts, $cmd, $value)) {
    ...
    private static function isRetryable($result, $retryCurlErrors, $options = [])
    {
        ...
        if (\count($retryCurlErrors) && ($previous = $result->getPrevious()) && $previous instanceof \WPStaging\Vendor\GuzzleHttp\Exception\RequestException) {
