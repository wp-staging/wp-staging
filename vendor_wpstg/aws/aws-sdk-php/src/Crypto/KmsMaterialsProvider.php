 ...
 * @deprecated
 */
class KmsMaterialsProvider extends \WPStaging\Vendor\Aws\Crypto\MaterialsProvider implements \WPStaging\Vendor\Aws\Crypto\MaterialsProviderInterface
{
    ...
    public function fromDecryptionEnvelope(\WPStaging\Vendor\Aws\Crypto\MetadataEnvelope $envelope)
    {
        if (empty($envelope[\WPStaging\Vendor\Aws\Crypto\MetadataEnvelope::MATERIALS_DESCRIPTION_HEADER])) {
            throw new \RuntimeException('Not able to detect the materials description.');
        }
        $materialsDescription = \json_decode($envelope[\WPStaging\Vendor\Aws\Crypto\MetadataEnvelope::MATERIALS_DESCRIPTION_HEADER], \true);
