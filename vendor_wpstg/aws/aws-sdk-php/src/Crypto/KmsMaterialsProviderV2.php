...
use WPStaging\Vendor\Aws\Kms\KmsClient;
/**
 ...
 */
class KmsMaterialsProviderV2 extends \WPStaging\Vendor\Aws\Crypto\MaterialsProviderV2 implements \WPStaging\Vendor\Aws\Crypto\MaterialsProviderInterfaceV2
{
     ...
     *                         and decrypting keys.
     */
    public function __construct(\WPStaging\Vendor\Aws\Kms\KmsClient $kmsClient, $kmsKeyId = null)
    {
    ...
    public function decryptCek($encryptedCek, $materialDescription, $options)
    {
        ...
        if (empty($options['@KmsAllowDecryptWithAnyCmk'])) {
            if (empty($this->kmsKeyId)) {
                throw new \WPStaging\Vendor\Aws\Exception\CryptoException('KMS CMK ID was not specified and the' . ' operation is not opted-in to attempting to use any valid' . ' CMK it discovers. Please specify a CMK ID, or explicitly' . ' enable attempts to use any valid KMS CMK with the' . ' @KmsAllowDecryptWithAnyCmk option.');
            }
    ...
    {
        if (empty($this->kmsKeyId)) {
            throw new \WPStaging\Vendor\Aws\Exception\CryptoException('A KMS key id is required for encryption' . ' with KMS keywrap. Use a KmsMaterialsProviderV2 that has been' . ' instantiated with a KMS key id.');
        }
        $options = \array_change_key_case($options);
        if (!isset($options['@kmsencryptioncontext']) || !\is_array($options['@kmsencryptioncontext'])) {
            throw new \WPStaging\Vendor\Aws\Exception\CryptoException("'@KmsEncryptionContext' is a" . " required argument when using KmsMaterialsProviderV2, and" . " must be an associative array (or empty array).");
        }
        if (isset($options['@kmsencryptioncontext']['aws:x-amz-cek-alg'])) {
            throw new \WPStaging\Vendor\Aws\Exception\CryptoException("Conflict in reserved @KmsEncryptionContext" . " key aws:x-amz-cek-alg. This value is reserved for the S3" . " Encryption Client and cannot be set by the user.");
        }
