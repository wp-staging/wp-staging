...
namespace WPStaging\Vendor\Aws\Crypto\Cipher;
...
trait CipherBuilderTrait
{
    ...
    protected function buildCipherMethod($cipherName, $iv, $keySize)
    {
        switch ($cipherName) {
            case 'cbc':
                return new \WPStaging\Vendor\Aws\Crypto\Cipher\Cbc($iv, $keySize);
            default:
                return null;
        }
    }
    ...
    protected function getCipherFromAesName($aesName)
    {
        switch ($aesName) {
            ...
            case 'AES/CBC/PKCS5Padding':
                return 'cbc';
            default:
                throw new \WPStaging\Vendor\Aws\Exception\CryptoException('Unrecognized or unsupported' . ' AESName for reverse lookup.');
        }
    }
}
