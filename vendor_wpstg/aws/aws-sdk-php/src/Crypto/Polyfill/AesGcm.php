...
use WPStaging\Vendor\Aws\Exception\CryptoPolyfillException;
...
class AesGcm
{
     ...
     */
    public function __construct(\WPStaging\Vendor\Aws\Crypto\Polyfill\Key $aesKey, $keySize = 256, $blockSize = 8192)
    {
        /* Preconditions: */
     ...
     * @throws InvalidArgumentException
     */
    public static function encrypt($plaintext, $nonce, \WPStaging\Vendor\Aws\Crypto\Polyfill\Key $key, $aad, &$tag, $keySize = 256, $blockSize = 8192)
    {
        self::needs(self::strlen($nonce) === 12, 'Nonce must be exactly 12 bytes', \InvalidArgumentException::class);
        $encryptor = new \WPStaging\Vendor\Aws\Crypto\Polyfill\AesGcm($key, $keySize, $blockSize);
        ...
        for ($i = 0; $i < $ciphertextLength; $i += $encryptor->blockSize) {
            $cBlock = new \WPStaging\Vendor\Aws\Crypto\Polyfill\ByteArray(self::substr($ciphertext, $i, $encryptor->blockSize));
            $gmac->update($cBlock);
        }
     ...
     * @throws InvalidArgumentException
     */
    public static function decrypt($ciphertext, $nonce, \WPStaging\Vendor\Aws\Crypto\Polyfill\Key $key, $aad, &$tag, $keySize = 256, $blockSize = 8192)
    {
        /* Precondition: */
        self::needs(self::strlen($nonce) === 12, 'Nonce must be exactly 12 bytes', \InvalidArgumentException::class);
        $encryptor = new \WPStaging\Vendor\Aws\Crypto\Polyfill\AesGcm($key, $keySize, $blockSize);
        ...
        $expected = new \WPStaging\Vendor\Aws\Crypto\Polyfill\ByteArray($tag);
    ...
    {
        $gmac = new \WPStaging\Vendor\Aws\Crypto\Polyfill\Gmac($this->aesKey, $nonce . "\0\0\0\1", $this->keySize);
        $aadBlock = new \WPStaging\Vendor\Aws\Crypto\Polyfill\ByteArray($aad);
