...
class ByteArray extends \SplFixedArray
{
    ...
    public function __construct($size = 0)
    {
        ...
        if (\is_array($size)) {
            ...
            $size = \count($arr);
        } elseif ($size instanceof \WPStaging\Vendor\Aws\Crypto\Polyfill\ByteArray) {
            $arr = $size->toArray();
            $size = $size->count();
        } elseif (!\is_int($size)) {
    ...
    {
        return new \WPStaging\Vendor\Aws\Crypto\Polyfill\ByteArray(\pack('N', $num));
    }
    /**
     * @param ByteArray $other
     * @return bool
     */
    public function equals(\WPStaging\Vendor\Aws\Crypto\Polyfill\ByteArray $other)
    {
     ...
     * @return ByteArray
     */
    public function exclusiveOr(\WPStaging\Vendor\Aws\Crypto\Polyfill\ByteArray $array)
    {
     ...
     * @return ByteArray
     */
    public static function select($select, \WPStaging\Vendor\Aws\Crypto\Polyfill\ByteArray $left, \WPStaging\Vendor\Aws\Crypto\Polyfill\ByteArray $right)
    {
     ...
     * @return self
     */
    public function set(\WPStaging\Vendor\Aws\Crypto\Polyfill\ByteArray $input, $offset = 0, $length = null)
    {
    ...
    {
        return new \WPStaging\Vendor\Aws\Crypto\Polyfill\ByteArray(\array_slice($this->toArray(), $start, $length));
    }
    /**
