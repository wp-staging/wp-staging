...
use WPStaging\Vendor\GuzzleHttp\Psr7\AppendStream;
use WPStaging\Vendor\GuzzleHttp\Psr7\Stream;
trait EncryptionTrait
{
     ...
     * @internal
     */
    public function encrypt(\WPStaging\Vendor\GuzzleHttp\Psr7\Stream $plaintext, array $cipherOptions, \WPStaging\Vendor\Aws\Crypto\MaterialsProvider $provider, \WPStaging\Vendor\Aws\Crypto\MetadataEnvelope $envelope)
    {
        ...
        if (!\WPStaging\Vendor\Aws\Crypto\MaterialsProvider::isSupportedKeySize($cipherOptions['KeySize'])) {
        ...
        $envelope[\WPStaging\Vendor\Aws\Crypto\MetadataEnvelope::CONTENT_KEY_V2_HEADER] = $provider->encryptCek($cek, $materialsDescription);
        unset($cek);
        $envelope[\WPStaging\Vendor\Aws\Crypto\MetadataEnvelope::IV_HEADER] = \base64_encode($cipherOptions['Iv']);
        $envelope[\WPStaging\Vendor\Aws\Crypto\MetadataEnvelope::KEY_WRAP_ALGORITHM_HEADER] = $provider->getWrapAlgorithmName();
        $envelope[\WPStaging\Vendor\Aws\Crypto\MetadataEnvelope::CONTENT_CRYPTO_SCHEME_HEADER] = $aesName;
        $envelope[\WPStaging\Vendor\Aws\Crypto\MetadataEnvelope::UNENCRYPTED_CONTENT_LENGTH_HEADER] = \strlen($plaintext);
        $envelope[\WPStaging\Vendor\Aws\Crypto\MetadataEnvelope::MATERIALS_DESCRIPTION_HEADER] = \json_encode($materialsDescription);
        if (!empty($cipherOptions['Tag'])) {
            $envelope[\WPStaging\Vendor\Aws\Crypto\MetadataEnvelope::CRYPTO_TAG_LENGTH_HEADER] = \strlen($cipherOptions['Tag']) * 8;
        }
     ...
     * @internal
     */
    protected function getEncryptingStream(\WPStaging\Vendor\GuzzleHttp\Psr7\Stream $plaintext, $cek, &$cipherOptions)
    {
        switch ($cipherOptions['Cipher']) {
            case 'gcm':
                $cipherOptions['TagLength'] = 16;
                $cipherTextStream = new \WPStaging\Vendor\Aws\Crypto\AesGcmEncryptingStream($plaintext, $cek, $cipherOptions['Iv'], $cipherOptions['Aad'] = isset($cipherOptions['Aad']) ? $cipherOptions['Aad'] : null, $cipherOptions['TagLength'], $cipherOptions['KeySize']);
                ...
                $appendStream = new \WPStaging\Vendor\GuzzleHttp\Psr7\AppendStream([$cipherTextStream->createStream()]);
                $cipherOptions['Tag'] = $cipherTextStream->getTag();
                $appendStream->addStream(\WPStaging\Vendor\GuzzleHttp\Psr7\Utils::streamFor($cipherOptions['Tag']));
            ...
            default:
                ...
                $cipherTextStream = new \WPStaging\Vendor\Aws\Crypto\AesEncryptingStream($plaintext, $cek, $cipherMethod);
