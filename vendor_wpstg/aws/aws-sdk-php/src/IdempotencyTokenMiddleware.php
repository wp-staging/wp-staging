...
class IdempotencyTokenMiddleware
{
     ...
     * @return callable
     */
    public static function wrap(\WPStaging\Vendor\Aws\Api\Service $service, callable $bytesGenerator = null)
    {
        return function (callable $handler) use($service, $bytesGenerator) {
            return new self($handler, $service, $bytesGenerator);
        };
    }
    public function __construct(callable $nextHandler, \WPStaging\Vendor\Aws\Api\Service $service, callable $bytesGenerator = null)
    {
