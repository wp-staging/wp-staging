...
class ConfigurationProvider extends \WPStaging\Vendor\Aws\AbstractConfigurationProvider implements \WPStaging\Vendor\Aws\ConfigurationProviderInterface
{
    ...
    protected static $interfaceClass = \WPStaging\Vendor\Aws\Endpoint\UseDualstackEndpoint\ConfigurationInterface::class;
    protected static $exceptionClass = \WPStaging\Vendor\Aws\Endpoint\UseDualstackEndpoint\Exception\ConfigurationException::class;
    /**
    ...
    public static function env($region)
    {
        return function () use($region) {
            ...
            if (!empty($useDualstackEndpoint)) {
                return \WPStaging\Vendor\GuzzleHttp\Promise\Create::promiseFor(new \WPStaging\Vendor\Aws\Endpoint\UseDualstackEndpoint\Configuration($useDualstackEndpoint, $region));
            }
    ...
    public static function ini($region, $profile = null, $filename = null)
    {
        ...
        return function () use($region, $profile, $filename) {
            ...
            return \WPStaging\Vendor\GuzzleHttp\Promise\Create::promiseFor(new \WPStaging\Vendor\Aws\Endpoint\UseDualstackEndpoint\Configuration($data[$profile][self::INI_USE_DUAL_STACK_ENDPOINT], $region));
        };
    }
    /**
    ...
    {
        return function () use($region) {
            return \WPStaging\Vendor\GuzzleHttp\Promise\Create::promiseFor(new \WPStaging\Vendor\Aws\Endpoint\UseDualstackEndpoint\Configuration(\false, $region));
        };
    }
}
