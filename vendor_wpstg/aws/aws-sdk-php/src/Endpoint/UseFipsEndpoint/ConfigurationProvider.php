...
class ConfigurationProvider extends \WPStaging\Vendor\Aws\AbstractConfigurationProvider implements \WPStaging\Vendor\Aws\ConfigurationProviderInterface
{
    ...
    protected static $interfaceClass = \WPStaging\Vendor\Aws\Endpoint\UseFipsEndpoint\ConfigurationInterface::class;
    protected static $exceptionClass = \WPStaging\Vendor\Aws\Endpoint\UseFipsEndpoint\Exception\ConfigurationException::class;
    /**
    ...
    public static function env()
    {
        return function () {
            ...
            if (!empty($useFipsEndpoint)) {
                return \WPStaging\Vendor\GuzzleHttp\Promise\Create::promiseFor(new \WPStaging\Vendor\Aws\Endpoint\UseFipsEndpoint\Configuration($useFipsEndpoint));
            }
    ...
    public static function ini($profile = null, $filename = null)
    {
        ...
        return function () use($profile, $filename) {
            ...
            $data = \WPStaging\Vendor\Aws\parse_ini_file($filename, \true, \INI_SCANNER_NORMAL);
            if ($data === \false) {
            ...
            return \WPStaging\Vendor\GuzzleHttp\Promise\Create::promiseFor(new \WPStaging\Vendor\Aws\Endpoint\UseFipsEndpoint\Configuration($data[$profile][self::INI_USE_FIPS_ENDPOINT]));
        };
    }
    /**
    ...
    public static function fallback($region)
    {
        return function () use($region) {
            ...
            if ($isFipsPseudoRegion) {
                $configuration = new \WPStaging\Vendor\Aws\Endpoint\UseFipsEndpoint\Configuration(\true);
            } else {
                $configuration = new \WPStaging\Vendor\Aws\Endpoint\UseFipsEndpoint\Configuration(\false);
            }
            return \WPStaging\Vendor\GuzzleHttp\Promise\Create::promiseFor($configuration);
        };
    }
}
