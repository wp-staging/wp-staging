...
namespace WPStaging\Vendor\Aws\Endpoint;

use ArrayAccess;
use WPStaging\Vendor\Aws\HasDataTrait;
use WPStaging\Vendor\Aws\Sts\RegionalEndpoints\ConfigurationProvider;
use WPStaging\Vendor\Aws\S3\RegionalEndpoint\ConfigurationProvider as S3ConfigurationProvider;
use InvalidArgumentException as Iae;
/**
 * Default implementation of an AWS partition.
 */
final class Partition implements \ArrayAccess, \WPStaging\Vendor\Aws\Endpoint\PartitionInterface
{
    use HasDataTrait;
    ...
    {
        return $service === 'sts' && \in_array($region, $this->stsLegacyGlobalRegions) && (empty($options['sts_regional_endpoints']) || \WPStaging\Vendor\Aws\Sts\RegionalEndpoints\ConfigurationProvider::unwrap($options['sts_regional_endpoints'])->getEndpointsType() !== 'regional');
    }
    /**
    ...
    {
        return $service === 's3' && $region === 'us-east-1' && (empty($options['s3_us_east_1_regional_endpoint']) || \WPStaging\Vendor\Aws\S3\RegionalEndpoint\ConfigurationProvider::unwrap($options['s3_us_east_1_regional_endpoint'])->getEndpointsType() !== 'regional');
    }
