...
use WPStaging\Vendor\Aws\AbstractConfigurationProvider;
use WPStaging\Vendor\Aws\CacheInterface;
use WPStaging\Vendor\Aws\ClientSideMonitoring\Exception\ConfigurationException;
use WPStaging\Vendor\Aws\ConfigurationProviderInterface;
 ...
 * </code>
 */
class ConfigurationProvider extends \WPStaging\Vendor\Aws\AbstractConfigurationProvider implements \WPStaging\Vendor\Aws\ConfigurationProviderInterface
{
    ...
    protected static $interfaceClass = \WPStaging\Vendor\Aws\ClientSideMonitoring\ConfigurationInterface::class;
    protected static $exceptionClass = \WPStaging\Vendor\Aws\ClientSideMonitoring\Exception\ConfigurationException::class;
    /**
    ...
    public static function env()
    {
        return function () {
            ...
            if ($enabled !== \false) {
                return \WPStaging\Vendor\GuzzleHttp\Promise\Create::promiseFor(new \WPStaging\Vendor\Aws\ClientSideMonitoring\Configuration($enabled, \getenv(self::ENV_HOST) ?: self::DEFAULT_HOST, \getenv(self::ENV_PORT) ?: self::DEFAULT_PORT, \getenv(self::ENV_CLIENT_ID) ?: self::DEFAULT_CLIENT_ID));
            }
    ...
    {
        return function () {
            return \WPStaging\Vendor\GuzzleHttp\Promise\Create::promiseFor(new \WPStaging\Vendor\Aws\ClientSideMonitoring\Configuration(self::DEFAULT_ENABLED, self::DEFAULT_HOST, self::DEFAULT_PORT, self::DEFAULT_CLIENT_ID));
        };
    }
    /**
    ...
    public static function ini($profile = null, $filename = null)
    {
        ...
        return function () use($profile, $filename) {
            ...
            $data = \WPStaging\Vendor\Aws\parse_ini_file($filename, \true);
            if ($data === \false) {
            ...
            return \WPStaging\Vendor\GuzzleHttp\Promise\Create::promiseFor(new \WPStaging\Vendor\Aws\ClientSideMonitoring\Configuration($data[$profile]['csm_enabled'], $data[$profile]['csm_host'], $data[$profile]['csm_port'], $data[$profile]['csm_client_id']));
        };
    }
    /**
    ...
    public static function unwrap($config)
    {
        ...
        if ($config instanceof \WPStaging\Vendor\GuzzleHttp\Promise\PromiseInterface) {
            $config = $config->wait();
        }
        if ($config instanceof \WPStaging\Vendor\Aws\ClientSideMonitoring\ConfigurationInterface) {
            return $config;
            ...
            return new \WPStaging\Vendor\Aws\ClientSideMonitoring\Configuration($config['enabled'], $host, $port, $client_id);
        }
