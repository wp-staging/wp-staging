 ...
 * @internal
 */
class ApiCallAttemptMonitoringMiddleware extends \WPStaging\Vendor\Aws\ClientSideMonitoring\AbstractMonitoringMiddleware
{
    /**
    ...
    {
        if ($klass instanceof \WPStaging\Vendor\Aws\ResultInterface) {
        ...
        if ($klass instanceof \WPStaging\Vendor\Aws\Exception\AwsException) {
    ...
    private static function getResultAttemptLatency(\WPStaging\Vendor\Aws\ResultInterface $result)
    {
    ...
    private static function getResultDestinationIp(\WPStaging\Vendor\Aws\ResultInterface $result)
    {
        if (isset($result['@metadata']['transferStats']['http'])) {
            $attempt = \end($result['@metadata']['transferStats']['http']);
    ...
    private static function getResultDnsLatency(\WPStaging\Vendor\Aws\ResultInterface $result)
    {
        if (isset($result['@metadata']['transferStats']['http'])) {
            $attempt = \end($result['@metadata']['transferStats']['http']);
    ...
    private static function getResultHttpStatusCode(\WPStaging\Vendor\Aws\ResultInterface $result)
    {
        return $result['@metadata']['statusCode'];
    }
    private static function getAwsExceptionAttemptLatency(\WPStaging\Vendor\Aws\Exception\AwsException $e)
    {
    ...
    private static function getAwsExceptionErrorCode(\WPStaging\Vendor\Aws\Exception\AwsException $e)
    {
        return $e->getAwsErrorCode();
    }
    private static function getAwsExceptionMessage(\WPStaging\Vendor\Aws\Exception\AwsException $e)
    {
        return $e->getAwsErrorMessage();
    }
    private static function getAwsExceptionDestinationIp(\WPStaging\Vendor\Aws\Exception\AwsException $e)
    {
        $attempt = $e->getTransferInfo();
        if (isset($attempt['primary_ip'])) {
            return $attempt['primary_ip'];
    ...
    private static function getAwsExceptionDnsLatency(\WPStaging\Vendor\Aws\Exception\AwsException $e)
    {
        $attempt = $e->getTransferInfo();
        if (isset($attempt['namelookup_time'])) {
            return (int) \floor($attempt['namelookup_time'] * 1000);
    ...
    private static function getAwsExceptionHttpStatusCode(\WPStaging\Vendor\Aws\Exception\AwsException $e)
    {
