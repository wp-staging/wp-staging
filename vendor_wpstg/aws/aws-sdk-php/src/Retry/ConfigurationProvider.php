...
use WPStaging\Vendor\Aws\Retry\Exception\ConfigurationException;
...
class ConfigurationProvider extends \WPStaging\Vendor\Aws\AbstractConfigurationProvider implements \WPStaging\Vendor\Aws\ConfigurationProviderInterface
{
    ...
    protected static $interfaceClass = \WPStaging\Vendor\Aws\Retry\ConfigurationInterface::class;
    protected static $exceptionClass = \WPStaging\Vendor\Aws\Retry\Exception\ConfigurationException::class;
    /**
    ...
    public static function env()
    {
        return function () {
            ...
            if (!empty($mode)) {
                return \WPStaging\Vendor\GuzzleHttp\Promise\Create::promiseFor(new \WPStaging\Vendor\Aws\Retry\Configuration($mode, $maxAttempts));
            }
    ...
    {
        return function () {
            return \WPStaging\Vendor\GuzzleHttp\Promise\Create::promiseFor(new \WPStaging\Vendor\Aws\Retry\Configuration(self::DEFAULT_MODE, self::DEFAULT_MAX_ATTEMPTS));
        };
    }
    /**
    ...
    public static function ini($profile = null, $filename = null)
    {
        ...
        return function () use($profile, $filename) {
            ...
            return \WPStaging\Vendor\GuzzleHttp\Promise\Create::promiseFor(new \WPStaging\Vendor\Aws\Retry\Configuration($data[$profile][self::INI_MODE], $maxAttempts));
        };
    }
    /**
    ...
    public static function unwrap($config)
    {
        ...
        if ($config instanceof \WPStaging\Vendor\Aws\Retry\ConfigurationInterface) {
            return $config;
        }
        ...
        if (\is_int($config)) {
            return new \WPStaging\Vendor\Aws\Retry\Configuration('legacy', $config + 1);
        }
        if (\is_array($config) && isset($config['mode'])) {
            $maxAttempts = isset($config['max_attempts']) ? $config['max_attempts'] : self::DEFAULT_MAX_ATTEMPTS;
            return new \WPStaging\Vendor\Aws\Retry\Configuration($config['mode'], $maxAttempts);
        }
