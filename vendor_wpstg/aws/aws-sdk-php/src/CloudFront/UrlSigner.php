...
class UrlSigner
{
    ...
    {
        $this->signer = new \WPStaging\Vendor\Aws\CloudFront\Signer($keyPairId, $privateKey);
    }
    /**
    ...
    public function getSignedUrl($url, $expires = null, $policy = null)
    {
        ...
        $uri = new \WPStaging\Vendor\GuzzleHttp\Psr7\Uri($scheme . '://' . $urlSections[1]);
        $query = \WPStaging\Vendor\GuzzleHttp\Psr7\Query::parse($uri->getQuery(), \PHP_QUERY_RFC3986);
    ...
    private function createRtmpUrl(\WPStaging\Vendor\Psr\Http\Message\UriInterface $uri)
    {
