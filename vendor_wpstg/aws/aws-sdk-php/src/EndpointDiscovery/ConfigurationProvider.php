...
use WPStaging\Vendor\Aws\EndpointDiscovery\Exception\ConfigurationException;
...
class ConfigurationProvider extends \WPStaging\Vendor\Aws\AbstractConfigurationProvider implements \WPStaging\Vendor\Aws\ConfigurationProviderInterface
{
    ...
    protected static $interfaceClass = \WPStaging\Vendor\Aws\EndpointDiscovery\ConfigurationInterface::class;
    protected static $exceptionClass = \WPStaging\Vendor\Aws\EndpointDiscovery\Exception\ConfigurationException::class;
    /**
    ...
    public static function defaultProvider(array $config = [])
    {
        ...
        if (isset($config['endpoint_discovery']) && $config['endpoint_discovery'] instanceof \WPStaging\Vendor\Aws\CacheInterface) {
    ...
    public static function env($cacheLimit = self::DEFAULT_CACHE_LIMIT)
    {
        return function () use($cacheLimit) {
            ...
            if ($enabled !== \false && $enabled !== '') {
                return \WPStaging\Vendor\GuzzleHttp\Promise\Create::promiseFor(new \WPStaging\Vendor\Aws\EndpointDiscovery\Configuration($enabled, $cacheLimit));
            }
    ...
    public static function fallback($config = [])
    {
        ...
        return function () use($enabled) {
            return \WPStaging\Vendor\GuzzleHttp\Promise\Create::promiseFor(new \WPStaging\Vendor\Aws\EndpointDiscovery\Configuration($enabled, self::DEFAULT_CACHE_LIMIT));
        };
    }
    /**
    ...
    public static function ini($profile = null, $filename = null, $cacheLimit = self::DEFAULT_CACHE_LIMIT)
    {
        ...
        return function () use($profile, $filename, $cacheLimit) {
            ...
            return \WPStaging\Vendor\GuzzleHttp\Promise\Create::promiseFor(new \WPStaging\Vendor\Aws\EndpointDiscovery\Configuration($data[$profile]['endpoint_discovery_enabled'], $cacheLimit));
        };
    }
    /**
    ...
    public static function unwrap($config)
    {
        ...
        if ($config instanceof \WPStaging\Vendor\Aws\EndpointDiscovery\ConfigurationInterface) {
            return $config;
        } elseif (\is_array($config) && isset($config['enabled'])) {
            if (isset($config['cache_limit'])) {
                return new \WPStaging\Vendor\Aws\EndpointDiscovery\Configuration($config['enabled'], $config['cache_limit']);
            }
            return new \WPStaging\Vendor\Aws\EndpointDiscovery\Configuration($config['enabled'], self::DEFAULT_CACHE_LIMIT);
        }
