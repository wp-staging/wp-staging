...
class ResultPaginator implements \Iterator
{
     ...
     * @param array              $config
     */
    public function __construct(\WPStaging\Vendor\Aws\AwsClientInterface $client, $operation, array $args, array $config)
    {
        $this->client = $client;
    ...
    {
        return \WPStaging\Vendor\GuzzleHttp\Promise\Coroutine::of(function () use($handleResult) {
            $nextToken = null;
            do {
                ...
                if ($retVal !== null) {
                    (yield \WPStaging\Vendor\GuzzleHttp\Promise\Create::promiseFor($retVal));
                }
            } while ($nextToken);
        });
    }
    ...
    public function search($expression)
    {
        ...
        return flatmap($this, function (\WPStaging\Vendor\Aws\Result $result) use($expression) {
    ...
    private function determineNextToken(\WPStaging\Vendor\Aws\Result $result)
    {
