...
use WPStaging\Vendor\GuzzleHttp\Event\EndEvent;
use WPStaging\Vendor\GuzzleHttp\Exception\ConnectException;
use WPStaging\Vendor\GuzzleHttp\Exception\RequestException;
use WPStaging\Vendor\GuzzleHttp\Message\ResponseInterface as GuzzleResponse;
use WPStaging\Vendor\GuzzleHttp\Promise;
use WPStaging\Vendor\GuzzleHttp\Psr7\Response as Psr7Response;
use WPStaging\Vendor\GuzzleHttp\Stream\Stream;
...
class GuzzleHandler
{
    ...
    public function __invoke(\WPStaging\Vendor\Psr\Http\Message\RequestInterface $request, array $options = [])
    {
        ...
        $promise = new \WPStaging\Vendor\GuzzleHttp\Promise\Promise(function () use($guzzlePromise) {
            try {
                $guzzlePromise->wait();
            } catch (\Exception $e) {
        ...
        return $promise->then(function (\WPStaging\Vendor\GuzzleHttp\Message\ResponseInterface $response) {
            ...
            if ($exception instanceof \WPStaging\Vendor\GuzzleHttp\Exception\RequestException) {
                if (isset($options['sink'])) {
                    if (!$options['sink'] instanceof \WPStaging\Vendor\Psr\Http\Message\StreamInterface) {
                        $exception->getResponse()->setBody(\WPStaging\Vendor\GuzzleHttp\Stream\Stream::factory(\file_get_contents($options['sink'])));
                    }
                }
            }
            // Reject with information about the error.
            return new \WPStaging\Vendor\GuzzleHttp\Promise\RejectedPromise($this->prepareErrorData($exception));
        });
    }
    private function createGuzzleRequest(\WPStaging\Vendor\Psr\Http\Message\RequestInterface $psrRequest, array $options)
    {
        $ringConfig = [];
        ...
        // Prepare sink option.
        if (isset($options['sink'])) {
            $ringConfig['save_to'] = $options['sink'] instanceof \WPStaging\Vendor\Psr\Http\Message\StreamInterface ? new \WPStaging\Vendor\Aws\Handler\GuzzleV5\GuzzleStream($options['sink']) : $options['sink'];
            unset($options['sink']);
        }
        ...
        if (\is_callable($statsCallback)) {
            $request->getEmitter()->on('end', function (\WPStaging\Vendor\GuzzleHttp\Event\EndEvent $event) use($statsCallback) {
        ...
        if ($body->getSize() === 0) {
            ...
            $request->setBody(new \WPStaging\Vendor\Aws\Handler\GuzzleV5\GuzzleStream($body));
        }
        $request->setHeaders($psrRequest->getHeaders());
        $request->setHeader('User-Agent', $request->getHeader('User-Agent') . ' ' . \WPStaging\Vendor\GuzzleHttp\Client::getDefaultUserAgent());
    ...
    private function createPsr7Response(\WPStaging\Vendor\GuzzleHttp\Message\ResponseInterface $response)
    {
        if ($body = $response->getBody()) {
            $body = new \WPStaging\Vendor\Aws\Handler\GuzzleV5\PsrStream($body);
        }
        return new \WPStaging\Vendor\GuzzleHttp\Psr7\Response($response->getStatusCode(), $response->getHeaders(), $body, $response->getReasonPhrase());
    }
    private function prepareErrorData(\Exception $e)
    {
        $error = ['exception' => $e, 'connection_error' => \false, 'response' => null];
        if ($e instanceof \WPStaging\Vendor\GuzzleHttp\Exception\ConnectException) {
