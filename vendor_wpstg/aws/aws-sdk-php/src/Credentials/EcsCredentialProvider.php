...
class EcsCredentialProvider
{
    ...
    {
        $client = $this->client;
        $request = new \WPStaging\Vendor\GuzzleHttp\Psr7\Request('GET', self::getEcsUri());
        $headers = $this->setHeaderForAuthToken();
        return $client($request, ['timeout' => $this->timeout, 'proxy' => '', 'headers' => $headers])->then(function (\WPStaging\Vendor\Psr\Http\Message\ResponseInterface $response) {
            $result = $this->decodeResult((string) $response->getBody());
            return new \WPStaging\Vendor\Aws\Credentials\Credentials($result['AccessKeyId'], $result['SecretAccessKey'], $result['Token'], \strtotime($result['Expiration']));
        })->otherwise(function ($reason) {
            $reason = \is_array($reason) ? $reason['exception'] : $reason;
            $msg = $reason->getMessage();
            throw new \WPStaging\Vendor\Aws\Exception\CredentialsException("Error retrieving credential from ECS ({$msg})");
        });
    }
    ...
    private function decodeResult($response)
    {
        ...
        if (!isset($result['AccessKeyId'])) {
            throw new \WPStaging\Vendor\Aws\Exception\CredentialsException('Unexpected ECS credential value');
        }
        return $result;
    }
}
