...
class CredentialProvider
{
    ...
    public static function defaultProvider(array $config = [])
    {
        ...
        if (isset($config['credentials']) && $config['credentials'] instanceof \WPStaging\Vendor\Aws\CacheInterface) {
     ...
     * @return callable
     */
    public static function fromCredentials(\WPStaging\Vendor\Aws\Credentials\CredentialsInterface $creds)
    {
        $promise = \WPStaging\Vendor\GuzzleHttp\Promise\Create::promiseFor($creds);
    ...
    public static function chain()
    {
        ...
        return function ($previousCreds = null) use($links) {
            ...
            while ($next = \array_shift($links)) {
                if ($next instanceof \WPStaging\Vendor\Aws\Credentials\InstanceProfileProvider && $previousCreds instanceof \WPStaging\Vendor\Aws\Credentials\Credentials) {
    ...
    public static function memoize(callable $provider)
    {
        return function () use($provider) {
            ...
            return $result->then(function (\WPStaging\Vendor\Aws\Credentials\CredentialsInterface $creds) use($provider, &$isConstant, &$result) {
    ...
    public static function cache(callable $provider, \WPStaging\Vendor\Aws\CacheInterface $cache, $cacheKey = null)
    {
        ...
        return function () use($provider, $cache, $cacheKey) {
            ...
            if ($found instanceof \WPStaging\Vendor\Aws\Credentials\CredentialsInterface && !$found->isExpired()) {
                return \WPStaging\Vendor\GuzzleHttp\Promise\Create::promiseFor($found);
            }
            return $provider()->then(function (\WPStaging\Vendor\Aws\Credentials\CredentialsInterface $creds) use($cache, $cacheKey) {
    ...
    public static function env()
    {
        return function () {
            ...
            if ($key && $secret) {
                return \WPStaging\Vendor\GuzzleHttp\Promise\Create::promiseFor(new \WPStaging\Vendor\Aws\Credentials\Credentials($key, $secret, \getenv(self::ENV_SESSION) ?: NULL));
            }
    ...
    {
        return new \WPStaging\Vendor\Aws\Credentials\InstanceProfileProvider($config);
    }
    /**
    ...
    public static function sso($ssoProfileName, $filename = null, $config = [])
    {
        ...
        return function () use($ssoProfileName, $filename, $config) {
            ...
            try {
                $expiration = (new \WPStaging\Vendor\Aws\Api\DateTimeResult($tokenData['expiresAt']))->getTimestamp();
            } catch (\Exception $e) {
            ...
            if (empty($config['ssoClient'])) {
                $ssoClient = new \WPStaging\Vendor\Aws\SSO\SSOClient(['region' => $ssoProfile['sso_region'], 'version' => '2019-06-10', 'credentials' => \false]);
            } else {
            ...
            return \WPStaging\Vendor\GuzzleHttp\Promise\Create::promiseFor(new \WPStaging\Vendor\Aws\Credentials\Credentials($ssoCredentials['accessKeyId'], $ssoCredentials['secretAccessKey'], $ssoCredentials['sessionToken'], $expiration));
        };
    }
    /**
    ...
    {
        return new \WPStaging\Vendor\Aws\Credentials\EcsCredentialProvider($config);
    }
    /**
    ...
    {
        return new \WPStaging\Vendor\Aws\Credentials\AssumeRoleCredentialProvider($config);
    }
    /**
    ...
    public static function assumeRoleWithWebIdentityCredentialProvider(array $config = [])
    {
        return function () use($config) {
            ...
            if ($tokenFromEnv && $arnFromEnv) {
                ...
                $provider = new \WPStaging\Vendor\Aws\Credentials\AssumeRoleWithWebIdentityCredentialProvider(['RoleArn' => $arnFromEnv, 'WebIdentityTokenFile' => $tokenFromEnv, 'SessionName' => $sessionName, 'client' => $stsClient, 'region' => $region]);
                return $provider();
            }
            $profileName = \getenv(self::ENV_PROFILE) ?: 'default';
            ...
            if (isset($profiles[$profileName])) {
                ...
                if (isset($profile['web_identity_token_file']) && isset($profile['role_arn'])) {
                    ...
                    $provider = new \WPStaging\Vendor\Aws\Credentials\AssumeRoleWithWebIdentityCredentialProvider(['RoleArn' => $profile['role_arn'], 'WebIdentityTokenFile' => $profile['web_identity_token_file'], 'SessionName' => $sessionName, 'client' => $stsClient, 'region' => $region]);
                    return $provider();
                }
            } else {
    ...
    public static function ini($profile = null, $filename = null, array $config = [])
    {
        ...
        return function () use($profile, $filename, $config) {
            ...
            */
            ...
            return \WPStaging\Vendor\GuzzleHttp\Promise\Create::promiseFor(new \WPStaging\Vendor\Aws\Credentials\Credentials($data[$profile]['aws_access_key_id'], $data[$profile]['aws_secret_access_key'], $data[$profile]['aws_session_token']));
        };
    }
    /**
    ...
    public static function process($profile = null, $filename = null)
    {
        ...
        return function () use($profile, $filename) {
            ...
            $data = \WPStaging\Vendor\Aws\parse_ini_file($filename, \true, \INI_SCANNER_RAW);
            if ($data === \false) {
                return self::reject("Invalid credentials file: {$filename}");
            }
            if (!isset($data[$profile])) {
            ...
            if (isset($processData['Expiration'])) {
                try {
                    $expiration = new \WPStaging\Vendor\Aws\Api\DateTimeResult($processData['Expiration']);
                } catch (\Exception $e) {
                    return self::reject("credential_process returned invalid expiration");
                }
                $now = new \WPStaging\Vendor\Aws\Api\DateTimeResult();
                if ($expiration < $now) {
            ...
            return \WPStaging\Vendor\GuzzleHttp\Promise\Create::promiseFor(new \WPStaging\Vendor\Aws\Credentials\Credentials($processData['AccessKeyId'], $processData['SecretAccessKey'], $processData['SessionToken'], $expires));
        };
    }
    /**
    ...
    private static function loadRoleProfile($profiles, $profileName, $filename, $stsClient, $config = [])
    {
        ...
        if (empty($stsClient)) {
            ...
            if (!empty($roleProfile['source_profile'])) {
                $sourceCredentials = \call_user_func(\WPStaging\Vendor\Aws\Credentials\CredentialProvider::ini($sourceProfileName, $filename, $config))->wait();
            } else {
                $sourceCredentials = self::getCredentialsFromSource($profileName, $filename);
            }
            $stsClient = new \WPStaging\Vendor\Aws\Sts\StsClient(['credentials' => $sourceCredentials, 'region' => $sourceRegion, 'version' => '2011-06-15']);
        }
        $result = $stsClient->assumeRole(['RoleArn' => $roleArn, 'RoleSessionName' => $roleSessionName]);
        $credentials = $stsClient->createCredentials($result);
        return \WPStaging\Vendor\GuzzleHttp\Promise\Create::promiseFor($credentials);
    }
    /**
    ...
    {
        $profileData = \WPStaging\Vendor\Aws\parse_ini_file($filename, \true, \INI_SCANNER_RAW);
        ...
        if ($filename === self::getHomeDir() . '/.aws/credentials' && \getenv('AWS_SDK_LOAD_NONDEFAULT_CONFIG')) {
            ...
            $configProfileData = \WPStaging\Vendor\Aws\parse_ini_file($configFilename, \true, \INI_SCANNER_RAW);
    ...
    private static function loadDefaultProfiles()
    {
        ...
        if (\file_exists($credFile)) {
            $profiles = \WPStaging\Vendor\Aws\parse_ini_file($credFile, \true, \INI_SCANNER_RAW);
        }
        if (\file_exists($configFile)) {
            $configProfileData = \WPStaging\Vendor\Aws\parse_ini_file($configFile, \true, \INI_SCANNER_RAW);
            foreach ($configProfileData as $name => $profile) {
                // standardize config profile names
                $name = \str_replace('profile ', '', $name);
    ...
    public static function getCredentialsFromSource($profileName = '', $filename = '', $config = [])
    {
        ...
        switch ($credentialSource) {
            ...
            case 'EcsContainer':
                ...
                break;
            default:
                throw new \WPStaging\Vendor\Aws\Exception\CredentialsException("Invalid credential_source found in config file: {$credentialSource}. Valid inputs " . "include Environment, Ec2InstanceMetadata, and EcsContainer.");
        }
        ...
        return function () use($credentialsResult) {
            return \WPStaging\Vendor\GuzzleHttp\Promise\Create::promiseFor($credentialsResult);
        };
    }
    private static function reject($msg)
    {
        return new \WPStaging\Vendor\GuzzleHttp\Promise\RejectedPromise(new \WPStaging\Vendor\Aws\Exception\CredentialsException($msg));
    }
    /**
     * @param $filename
     * @return string
    ...
    public static function shouldUseEcs()
    {
        ...
        return !empty(\getenv(\WPStaging\Vendor\Aws\Credentials\EcsCredentialProvider::ENV_URI)) || !empty($_SERVER[\WPStaging\Vendor\Aws\Credentials\EcsCredentialProvider::ENV_URI]) || !empty(\getenv(\WPStaging\Vendor\Aws\Credentials\EcsCredentialProvider::ENV_FULL_URI)) || !empty($_SERVER[\WPStaging\Vendor\Aws\Credentials\EcsCredentialProvider::ENV_FULL_URI]);
    }
}
