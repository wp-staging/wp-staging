...
abstract class AbstractUploader extends \WPStaging\Vendor\Aws\Multipart\AbstractUploadManager
{
     ...
     * @param array  $config
     */
    public function __construct(\WPStaging\Vendor\Aws\AwsClientInterface $client, $source, array $config = [])
    {
     ...
     */
    protected function limitPartStream(\WPStaging\Vendor\Psr\Http\Message\StreamInterface $stream)
    {
        // Limit what is read from the stream to the part size.
        return new \WPStaging\Vendor\GuzzleHttp\Psr7\LimitStream($stream, $this->state->getPartSize(), $this->source->tell());
    }
        ...
        for ($partNumber = 1; $this->isEof($seekable); $partNumber++) {
            ...
            if (!$this->state->hasPartBeenUploaded($partNumber)) {
                ...
                if (isset($numberOfParts) && $partNumber > $numberOfParts) {
                    throw new $this->config['exception_class']($this->state, new \WPStaging\Vendor\Aws\Exception\AwsException("Maximum part number for this job exceeded, file has likely been corrupted." . "  Please restart this upload.", $command));
                }
                (yield $command);
    ...
    private function determineSource($source)
    {
        ...
        if (\is_string($source)) {
            $source = \WPStaging\Vendor\GuzzleHttp\Psr7\Utils::tryFopen($source, 'r');
        }
        // Create a source stream.
        $stream = \WPStaging\Vendor\GuzzleHttp\Psr7\Utils::streamFor($source);
