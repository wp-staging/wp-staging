...
namespace WPStaging\Vendor\Aws\Multipart;

use WPStaging\Vendor\Aws\AwsClientInterface as Client;
use WPStaging\Vendor\Aws\CommandInterface;
use WPStaging\Vendor\Aws\CommandPool;
 ...
 * @internal
 */
abstract class AbstractUploadManager implements \WPStaging\Vendor\GuzzleHttp\Promise\PromisorInterface
{
    const DEFAULT_CONCURRENCY = 5;
    /** @var array Default values for base multipart configuration */
    private static $defaultConfig = ['part_size' => null, 'state' => null, 'concurrency' => self::DEFAULT_CONCURRENCY, 'prepare_data_source' => null, 'before_initiate' => null, 'before_upload' => null, 'before_complete' => null, 'exception_class' => 'WPStaging\\Vendor\\Aws\\Exception\\MultipartUploadException'];
     ...
     * @param array  $config
     */
    public function __construct(\WPStaging\Vendor\Aws\AwsClientInterface $client, array $config = [])
    {
        $this->client = $client;
    ...
    public function promise()
    {
        ...
        return $this->promise = \WPStaging\Vendor\GuzzleHttp\Promise\Coroutine::of(function () {
            // Initiate the upload.
            ...
            if (!$this->state->isInitiated()) {
                ...
                $this->state->setStatus(\WPStaging\Vendor\Aws\Multipart\UploadState::INITIATED);
            }
            ...
            $commands = new \WPStaging\Vendor\Aws\CommandPool($this->client, $this->getUploadCommands($resultHandler), ['concurrency' => $this->config['concurrency'], 'before' => $this->config['before_upload']]);
            ...
            $this->state->setStatus(\WPStaging\Vendor\Aws\Multipart\UploadState::COMPLETED);
     ...
     */
    protected abstract function handleResult(\WPStaging\Vendor\Aws\CommandInterface $command, \WPStaging\Vendor\Aws\ResultInterface $result);
    /**
    ...
    private function determineState()
    {
        ...
        if ($this->config['state'] instanceof \WPStaging\Vendor\Aws\Multipart\UploadState) {
        ...
        $state = new \WPStaging\Vendor\Aws\Multipart\UploadState($id);
    ...
    protected function getResultHandler(&$errors = [])
    {
        return function (callable $handler) use(&$errors) {
            return function (\WPStaging\Vendor\Aws\CommandInterface $command, \WPStaging\Vendor\Psr\Http\Message\RequestInterface $request = null) use($handler, &$errors) {
                return $handler($command, $request)->then(function (\WPStaging\Vendor\Aws\ResultInterface $result) use($command) {
                    $this->handleResult($command, $result);
                    return $result;
                }, function (\WPStaging\Vendor\Aws\Exception\AwsException $e) use(&$errors) {
                    $errors[$e->getCommand()[$this->info['part_num']]] = $e;
                    return new \WPStaging\Vendor\Aws\Result();
                });
            };
        };
    }
