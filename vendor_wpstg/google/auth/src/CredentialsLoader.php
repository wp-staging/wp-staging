...
use WPStaging\Vendor\Google\Auth\Credentials\InsecureCredentials;
 ...
 */
abstract class CredentialsLoader implements \WPStaging\Vendor\Google\Auth\FetchAuthTokenInterface, \WPStaging\Vendor\Google\Auth\UpdateMetadataInterface
{
    ...
    private static function getGuzzleMajorVersion()
    {
        if (\defined('WPStaging\\Vendor\\GuzzleHttp\\ClientInterface::MAJOR_VERSION')) {
            return \WPStaging\Vendor\GuzzleHttp\ClientInterface::MAJOR_VERSION;
        }
        if (\defined('WPStaging\\Vendor\\GuzzleHttp\\ClientInterface::VERSION')) {
            return (int) \substr(\WPStaging\Vendor\GuzzleHttp\ClientInterface::VERSION, 0, 1);
        }
    ...
    public static function makeCredentials($scope, array $jsonKey, $defaultScope = null)
    {
        ...
        if ($jsonKey['type'] == 'service_account') {
            ...
            return new \WPStaging\Vendor\Google\Auth\Credentials\ServiceAccountCredentials($scope, $jsonKey);
        }
        if ($jsonKey['type'] == 'authorized_user') {
            $anyScope = $scope ?: $defaultScope;
            return new \WPStaging\Vendor\Google\Auth\Credentials\UserRefreshCredentials($anyScope, $jsonKey);
        }
     ...
     */
    public static function makeHttpClient(\WPStaging\Vendor\Google\Auth\FetchAuthTokenInterface $fetcher, array $httpClientOptions = [], callable $httpHandler = null, callable $tokenCallback = null)
    {
        if (self::getGuzzleMajorVersion() === 5) {
            $client = new \WPStaging\Vendor\GuzzleHttp\Client($httpClientOptions);
            $client->setDefaultOption('auth', 'google_auth');
            $subscriber = new \WPStaging\Vendor\Google\Auth\Subscriber\AuthTokenSubscriber($fetcher, $httpHandler, $tokenCallback);
            $client->getEmitter()->attach($subscriber);
            return $client;
        }
        $middleware = new \WPStaging\Vendor\Google\Auth\Middleware\AuthTokenMiddleware($fetcher, $httpHandler, $tokenCallback);
        $stack = \WPStaging\Vendor\GuzzleHttp\HandlerStack::create();
        $stack->push($middleware);
        return new \WPStaging\Vendor\GuzzleHttp\Client(['handler' => $stack, 'auth' => 'google_auth'] + $httpClientOptions);
    }
    /**
    ...
    {
        return new \WPStaging\Vendor\Google\Auth\Credentials\InsecureCredentials();
    }
    /**
