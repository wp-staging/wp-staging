...
class SimpleMiddleware
{
    ...
    public function __invoke(callable $handler)
    {
        return function (\WPStaging\Vendor\Psr\Http\Message\RequestInterface $request, array $options) use($handler) {
            ...
            $query = \WPStaging\Vendor\GuzzleHttp\Psr7\Query::parse($request->getUri()->getQuery());
            $params = \array_merge($query, $this->config);
            $uri = $request->getUri()->withQuery(\WPStaging\Vendor\GuzzleHttp\Psr7\Query::build($params));
