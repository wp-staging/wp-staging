 ...
 */
namespace WPStaging\Vendor\Google\Auth\Middleware;

use WPStaging\Vendor\Google\Auth\FetchAuthTokenInterface;
...
class ProxyAuthTokenMiddleware
{
     ...
     */
    public function __construct(\WPStaging\Vendor\Google\Auth\FetchAuthTokenInterface $fetcher, callable $httpHandler = null, callable $tokenCallback = null)
    {
    ...
    {
        return function (\WPStaging\Vendor\Psr\Http\Message\RequestInterface $request, array $options) use($handler) {
            ...
            if ($quotaProject = $this->getQuotaProject()) {
                $request = $request->withHeader(\WPStaging\Vendor\Google\Auth\GetQuotaProjectInterface::X_GOOG_USER_PROJECT_HEADER, $quotaProject);
            }
            return $handler($request, $options);
        };
    }
