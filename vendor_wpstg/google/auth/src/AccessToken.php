...
use DateTime;
use Exception;
use WPStaging\Vendor\Firebase\JWT\ExpiredException;
use WPStaging\Vendor\Firebase\JWT\JWT;
...
use WPStaging\Vendor\GuzzleHttp\Psr7\Utils;
...
use WPStaging\Vendor\SimpleJWT\InvalidTokenException;
use WPStaging\Vendor\SimpleJWT\JWT as SimpleJWT;
use WPStaging\Vendor\SimpleJWT\Keys\KeyFactory;
use WPStaging\Vendor\SimpleJWT\Keys\KeySet;
...
class AccessToken
{
     ...
     */
    public function __construct(callable $httpHandler = null, \WPStaging\Vendor\Psr\Cache\CacheItemPoolInterface $cache = null)
    {
        $this->httpHandler = $httpHandler ?: \WPStaging\Vendor\Google\Auth\HttpHandler\HttpHandlerFactory::build(\WPStaging\Vendor\Google\Auth\HttpHandler\HttpClientCache::getHttpClient());
        $this->cache = $cache ?: new \WPStaging\Vendor\Google\Auth\Cache\MemoryCacheItemPool();
    }
    /**
    ...
    public function verify($token, array $options = [])
    {
        ...
        try {
            ...
            // firebase/php-jwt 3+
        } catch (\WPStaging\Vendor\SignatureInvalidException $e) {
            // firebase/php-jwt 2
        } catch (\WPStaging\Vendor\SimpleJWT\InvalidTokenException $e) {
            // simplejwt
        } catch (\WPStaging\Vendor\Google\Auth\DomainException $e) {
    ...
    {
        $this->checkSimpleJwt();
        $jwkset = new \WPStaging\Vendor\SimpleJWT\Keys\KeySet();
        foreach ($certs as $cert) {
            $jwkset->add(\WPStaging\Vendor\SimpleJWT\Keys\KeyFactory::create($cert, 'php'));
        }
    ...
    private function verifyRs256($token, array $certs, $audience = null, $issuer = null)
    {
        ...
        foreach ($certs as $cert) {
            ...
            $rsa->loadKey(['n' => new \WPStaging\Vendor\phpseclib\Math\BigInteger($this->callJwtStatic('urlsafeB64Decode', [$cert['n']]), 256), 'e' => new \WPStaging\Vendor\phpseclib\Math\BigInteger($this->callJwtStatic('urlsafeB64Decode', [$cert['e']]), 256)]);
    ...
    public function revoke($token, array $options = [])
    {
        ...
        $request = new \WPStaging\Vendor\GuzzleHttp\Psr7\Request('POST', self::OAUTH2_REVOKE_URI, ['Cache-Control' => 'no-store', 'Content-Type' => 'application/x-www-form-urlencoded'], $body);
    ...
    private function retrieveCertsFromLocation($url, array $options = [])
    {
        ...
        $httpHandler = $this->httpHandler;
        $response = $httpHandler(new \WPStaging\Vendor\GuzzleHttp\Psr7\Request('GET', $url), $options);
    ...
    {
        // @codeCoverageIgnoreStart
        if (!\class_exists('WPStaging\\Vendor\\SimpleJWT\\JWT')) {
    ...
    private function setPhpsecConstants()
    {
        if (\filter_var(\getenv('GAE_VM'), \FILTER_VALIDATE_BOOLEAN)) {
            ...
            if (!\defined('WPStaging\\Vendor\\CRYPT_RSA_MODE')) {
                \define('WPStaging\\Vendor\\CRYPT_RSA_MODE', \WPStaging\Vendor\phpseclib\Crypt\RSA::MODE_OPENSSL);
            }
        }
    }
    /**
    ...
    {
        $class = 'WPStaging\\Vendor\\Firebase\\JWT\\JWT';
