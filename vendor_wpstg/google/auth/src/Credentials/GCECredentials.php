 ...
 */
namespace WPStaging\Vendor\Google\Auth\Credentials;

use WPStaging\Vendor\Google\Auth\CredentialsLoader;
use WPStaging\Vendor\Google\Auth\GetQuotaProjectInterface;
...
use WPStaging\Vendor\Google\Auth\Iam;
use WPStaging\Vendor\Google\Auth\ProjectIdProviderInterface;
use WPStaging\Vendor\Google\Auth\SignBlobInterface;
...
use WPStaging\Vendor\GuzzleHttp\Exception\ServerException;
 ...
 */
class GCECredentials extends \WPStaging\Vendor\Google\Auth\CredentialsLoader implements \WPStaging\Vendor\Google\Auth\SignBlobInterface, \WPStaging\Vendor\Google\Auth\ProjectIdProviderInterface, \WPStaging\Vendor\Google\Auth\GetQuotaProjectInterface
{
    // phpcs:disable
     ...
     */
    public function __construct(\WPStaging\Vendor\Google\Auth\Iam $iam = null, $scope = null, $targetAudience = null, $quotaProject = null, $serviceAccountIdentity = null)
    {
        $this->iam = $iam;
    ...
    {
        $httpHandler = $httpHandler ?: \WPStaging\Vendor\Google\Auth\HttpHandler\HttpHandlerFactory::build(\WPStaging\Vendor\Google\Auth\HttpHandler\HttpClientCache::getHttpClient());
        ...
        for ($i = 1; $i <= self::MAX_COMPUTE_PING_TRIES; $i++) {
            try {
                ...
                // the metadata resolution was particularly slow. The latter case is
                // "unlikely".
                $resp = $httpHandler(new \WPStaging\Vendor\GuzzleHttp\Psr7\Request('GET', $checkUri, [self::FLAVOR_HEADER => 'Google']), ['timeout' => self::COMPUTE_PING_CONNECTION_TIMEOUT_S]);
                return $resp->getHeaderLine(self::FLAVOR_HEADER) == 'Google';
            } catch (\WPStaging\Vendor\GuzzleHttp\Exception\ClientException $e) {
            } catch (\WPStaging\Vendor\GuzzleHttp\Exception\ServerException $e) {
            } catch (\WPStaging\Vendor\GuzzleHttp\Exception\RequestException $e) {
            } catch (\WPStaging\Vendor\GuzzleHttp\Exception\ConnectException $e) {
            }
        }
        return \false;
    }
    ...
    {
        $httpHandler = \WPStaging\Vendor\Google\Auth\HttpHandler\HttpHandlerFactory::build(\WPStaging\Vendor\Google\Auth\HttpHandler\HttpClientCache::getHttpClient());
        ...
        $signer = $this->iam ?: new \WPStaging\Vendor\Google\Auth\Iam($httpHandler);
    ...
    {
        $resp = $httpHandler(new \WPStaging\Vendor\GuzzleHttp\Psr7\Request('GET', $uri, [self::FLAVOR_HEADER => 'Google']));
