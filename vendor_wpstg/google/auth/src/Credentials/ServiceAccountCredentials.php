...
use WPStaging\Vendor\Google\Auth\ServiceAccountSignerTrait;
 ...
 */
class ServiceAccountCredentials extends \WPStaging\Vendor\Google\Auth\CredentialsLoader implements \WPStaging\Vendor\Google\Auth\GetQuotaProjectInterface, \WPStaging\Vendor\Google\Auth\SignBlobInterface, \WPStaging\Vendor\Google\Auth\ProjectIdProviderInterface
{
    ...
    public function __construct($scope, $jsonKey, $sub = null, $targetAudience = null)
    {
        ...
        $this->auth = new \WPStaging\Vendor\Google\Auth\OAuth2(['audience' => self::TOKEN_CREDENTIAL_URI, 'issuer' => $jsonKey['client_email'], 'scope' => $scope, 'signingAlgorithm' => 'RS256', 'signingKey' => $jsonKey['private_key'], 'sub' => $sub, 'tokenCredentialUri' => self::TOKEN_CREDENTIAL_URI, 'additionalClaims' => $additionalClaims]);
    ...
    private function createJwtAccessCredentials()
    {
        if (!$this->jwtAccessCredentials) {
            ...
            $this->jwtAccessCredentials = new \WPStaging\Vendor\Google\Auth\Credentials\ServiceAccountJwtAccessCredentials($credJson, $this->auth->getScope());
        }
