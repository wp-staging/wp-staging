...
class GCECache
{
     ...
     */
    public function __construct(array $cacheConfig = null, \WPStaging\Vendor\Psr\Cache\CacheItemPoolInterface $cache = null)
    {
        $this->cache = $cache;
    ...
    public function onGce(callable $httpHandler = null)
    {
        if (\is_null($this->cache)) {
            return \WPStaging\Vendor\Google\Auth\Credentials\GCECredentials::onGce($httpHandler);
        }
        $cacheKey = self::GCE_CACHE_KEY;
        $onGce = $this->getCachedValue($cacheKey);
        if (\is_null($onGce)) {
            $onGce = \WPStaging\Vendor\Google\Auth\Credentials\GCECredentials::onGce($httpHandler);
