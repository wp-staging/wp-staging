...
use WPStaging\Vendor\GuzzleHttp\Psr7\Query;
 ...
 */
class OAuth2 implements \WPStaging\Vendor\Google\Auth\FetchAuthTokenInterface
{
    ...
    public function generateCredentialsRequest()
    {
        ...
        return new \WPStaging\Vendor\GuzzleHttp\Psr7\Request('POST', $uri, $headers, \WPStaging\Vendor\GuzzleHttp\Psr7\Query::build($params));
    }
    /**
     ...
     * @throws \Exception
     */
    public function parseTokenResponse(\WPStaging\Vendor\Psr\Http\Message\ResponseInterface $resp)
    {
    ...
    public function buildFullAuthorizationUri(array $config = [])
    {
        ...
        $existingParams = \WPStaging\Vendor\GuzzleHttp\Psr7\Query::parse($result->getQuery());
        $result = $result->withQuery(\WPStaging\Vendor\GuzzleHttp\Psr7\Query::build(\array_merge($existingParams, $params)));
    ...
    {
        return \WPStaging\Vendor\Firebase\JWT\JWT::decode($idToken, $publicKey, $allowedAlgs);
    }
    private function jwtEncode($assertion, $signingKey, $signingAlgorithm, $signingKeyId = null)
    {
        return \WPStaging\Vendor\Firebase\JWT\JWT::encode($assertion, $signingKey, $signingAlgorithm, $signingKeyId);
    }
    /**
