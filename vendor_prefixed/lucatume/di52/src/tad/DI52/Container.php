...
class tad_DI52_Container implements \ArrayAccess
{
    ...
    public function make($classOrInterface)
    {
        ...
        if (!isset($this->bindings[$classOrInterface])) {
            try {
                return $this->build($classOrInterface, \true);
            } catch (\Exception $e) {
    ...
    protected function build($implementation, $resolving = \false)
    {
        ...
        $instance = !empty($builtParams) ? $this->reflections[$implementation]->newInstanceArgs($builtParams) : new $implementation();
        return $instance;
    }
    /**
     * @param string $classOrInterface
    ...
    public function register($serviceProviderClass)
    {
        ...
        if (!$provider->isDeferred()) {
            ...
            $this->deferred = \array_merge($this->deferred, \array_combine($provided, \array_fill(0, $count, $provider)));
        }
    ...
    public function give($implementation)
    {
        ...
        $this->contexts[$this->neededImplementation] = !empty($this->contexts[$this->neededImplementation]) ? $this->contexts[$this->neededImplementation] : array();
    ...
    {
        return new \WPStaging\Vendor\tad_DI52_ProtectedValue($value);
    }
    /**
    ...
    public function callback($classOrInterface, $method)
    {
        ...
        if ($this->useClosures) {
            $f = \WPStaging\Vendor\di52_callbackClosure($this, $classOrInterface, $method);
        } else {
    ...
    public function _getParameter(\ReflectionParameter $parameter)
    {
        ...
        return isset($this->contexts[$parameterClass][$this->resolving]) ? $this->offsetGet($this->contexts[$parameterClass][$this->resolving]) : $this->offsetGet($parameterClass);
    }
    /**
    ...
    public function instance($classOrInterface, array $args = array())
    {
        ...
        if (!isset($this->instanceCallbacks[$instanceId])) {
            ...
            if ($this->useClosures) {
                $f = \WPStaging\Vendor\di52_instanceClosure($this, $classOrInterface, $args);
            } else {
                // @codeCoverageIgnoreStart
                if (\is_object($classOrInterface) || \is_callable($classOrInterface)) {
                    $objectId = \uniqid(\rand(1, 9999) . \md5($classOrInterfaceName));
